...
status: executing
stopped_at: PHASE 5 PLAN 18 EXECUTED (2026-08-17) -- THE FIFTH INSTANCE OF THE ENVELOPE-BOUNDARY CLASS IS CLOSED AND THE CLASS ITSELF IS NOW PINNED, WHICH IS WHAT MAKES THIS DIFFERENT FROM THE FOUR BEFORE IT. Task 1 (d249625, test, committed RED): police role, queue [FINAL_REVEAL, REVEAL], measured at HEAD da45b55 verbatim -- await_and_respond (police) returned type = final_reveal verdict = None; await_opponent_turn outcome = Outcome.TECHNICAL_LOSS; technical_win reasons = ['payload must be a dict, got NoneType']; queue left = 1, so the REVEAL we were actually waiting for was never read. turn_commit.py:103 called the pull primitive BARE with no type test and handed the peer's published ledger to move handling; decode_revealed_action wants a 'move' sub-key, the payload is {'records': [...]}, and reject_peer_payload turns that into OUR technical loss naming an honest peer -- a false declaration under rules 16/22, reachable in a league game. 05-17's buffer already had the ledger safe (audit_verdict matched=true held even at HEAD), so this was never about the audit: we lost a GAME we did not lose. Task 2 (f099ad2, fix): BOTH CANDIDATES WERE RUN, NOT ARGUED. Keep waiting -> outcome=None and zero accusations when the REVEAL follows, technical_loss + opponent_unresponsive when it never comes. End the game -> either AttributeError: NoneType object has no attribute sender (returning no envelope) or a FABRICATED technical_win against a peer that demonstrably just spoke. KEEP WAITING WINS BECAUSE ENDING THE GAME HAS NO TRUTHFUL EXPRESSION HERE: await_and_respond returns (Envelope|None, TechnicalWin|None), (None, None) crashes the caller, and the only other door is hand-building a TechnicalWin whose own docstring says every field is MEASURED by the retry ladder and never assumed or defaulted, so the declaration this carries is defensible at audit. Waiting costs nothing already owed -- the ledger is buffered, the REVEAL is normally the next queue item, and a peer that really stopped exhausts the ladder and earns D-13's own MEASURED opponent_unresponsive. NET-06 untouched; ZERO numeric values moved (05-17 had already measured the timing fix: 21 attempts, 10x shipped, byte-identical accusation). THE REPAIR REMOVES THE SECOND PATH RATHER THAN TEACHING IT THE SAME LESSON: both roles now run ONE leg, wait_for_reveal_capturing_early_ack, with h_commit: str | None where None is the initiator (initiate already collected this turn's ACK). A SECOND OMISSION AT THE SAME LINE, fixed with it: turn_commit.py:103 was the LAST production caller of next_protocol_message still taking on_attempt=None -- 05-16 marked the other five ladders and skipped this one for the same reason the type test was skipped. Measured pre-fix on the injected clock at shipped Table-19 values: freeze at attempt 2 of 4, t=70.0 s, touches=0, ProcessKilledError -- os._exit(1) MID-GAME and the D-13 verdict due at t=140 s never spoken. Task 3 (e01708c, test): THE CLASS GUARD. _pull_site_discovery.py walks src/pursuit/network/ with a two-stage AST rule -- SEED = a top-level function whose body INCLUDING NESTED CLOSURES reads the inbound queue, CLOSURE = fixpoint over callers of a set member whose RETURN ANNOTATION mentions Envelope. The annotation is what bounds it, so await_opponent_turn is IN and turn_commit.initiate is OUT. SITE COUNT: 12, and every one of the five historical instances (05-09, 05-10, 05-15, 05-17, #18) sits inside it. Invariant asserted three ways over 12 sites x 9 MessageType = 108 combinations: no traceback; every accusation a pull site writes is a TechnicalWinReason MEMBER and never a decoder's message; a peer's FINAL_REVEAL is returned, buffered or left queued but never destroyed. SIX PROBES, EVERY ONE WITH A REAL COUNT: A Task 2 reverted -> 8 BREACH ROWS on the shipped path; B enumeration pointed at a non-existent package -> AssertionError ZERO pull sites discovered, FAILS LOUDLY and is NOT a silent pytest skip; C one driver deleted -> undriven sites ['await_and_respond']; C2 A BRAND-NEW PULL FUNCTION DROPPED INTO THE PACKAGE -> undriven sites ['brand_new_pull'], so covered-on-the-day-it-lands is PROVEN not asserted; D deferred #17's exemption removed -> linger_for_peer fires, the exempted check is live; E kept forced True -> 12 sites kept the ledger, expected 12 discovered minus 1 exempt. RULE 36 SURVIVES INTACT, asserted in THREE modules and passing at HEAD BEFORE the fix as well as after -- a genuinely silent peer still earns technical_win{opponent_unresponsive}, and Task 2's own revert probe fails 3 of 4 police cases while that counter-control passes either way, which is what makes it a control. THE GUARD FOUND INSTANCE SIX ON ITS FIRST RUN, which is the whole argument for it: turn_buffer.await_move has NO type test of any kind, so on the commit_reveal=OFF path 8 of 9 MessageTypes come back 'payload has neither direction nor x/y keys' (ON is 0 of 9). Filed as deferred #19 and NOT fixed -- shipped config is commit_reveal true, turn_buffer.py is not in files_modified and sits at 146/150, and phase 5 ends here -- with a test that asserts the SCOPE and FAILS DELIBERATELY when #19 is closed, so the record cannot rot. SELF-AUDIT CAUGHT A GUARD OF MY OWN THAT NO TEST DEFENDED (f27b17f): the h_commit is not None guard, written in Task 2 and documented as load-bearing, could be DELETED with all 1538 tests still green -- the dict.get trap, where an ACK carrying no h_commit key makes None == None read True and a contentless ACK sets own_ack_received. Inert on police TODAY, which is exactly why nothing reported it. Pinned in both directions; probe F confirms it fires. IT ALSO CAUGHT AN ASSERTION OF MINE THAT COULD NOT FAIL -- len(sites) * len(MessageType) >= len(DRIVERS) is trivially 108 >= 12 -- replaced with the falsifiable positive count E probes. THREE 150-LINE SPLITS, never compressions: turn_commit_wait.py 135 -> 151 -> 122 (turn_commit_wait_reveal.py, along the division 05-18 itself created -- three role-specific legs stay, the one shared leg leaves; NOT re-exported, because it imports H_COMMIT_KEY from the parent and a re-export back would be an import CYCLE), test_envelope_boundary_invariant.py 151 -> 118 (test_toggle_off_move_boundary.py), and the guard pin into its own test_turn_commit_wait_reveal.py rather than take test_early_reveal_routing.py to 152. Deferred #18 CLOSED with re-measurement; #16 and #17 left OPEN with their measurements untouched per the plan's non-goals, and #17 is carried into the invariant as a NAMED exemption asserted to be a real discovered site. #20 NEW: three modules within two lines of the gate, seams named. GATES: ruff 0, line-limit exit 0 TREE-WIDE AND exit 0 AGAIN on every new file named EXPLICITLY (the no-argument form enumerates via git ls-files and passes VACUOUSLY over untracked files), no-llm OK, suite 1539 passed / 0 failed / 96.64% against the 1530 / 96.64% baseline (+9, exactly the nine added, coverage unchanged), dev_launch.py exit 0 with ONE shared uid afa6aa3840d63e88 across both logs, both sides audit_verdict matched=true over 5 peer turns, ZERO technical_win and ZERO watchdog_incident. test_late_peer_teardown PASSED throughout. Nothing weakened, nothing skipped, no --no-verify.
prior_stopped_at: PHASE 5 PLAN 17 EXECUTED (2026-08-17) -- WE MANUFACTURED THE SILENCE WE THEN PUNISHED, AND THE PUNISHMENT ITSELF IS UNCHANGED. A rules-16/22 false-declaration path reachable by an honest opponent doing nothing wrong is closed by ROUTING, with ZERO numeric values touched. Task 1 (b29a098, test, committed RED): a peer that finishes its turn loop first and pushes its ledger, after which our still-running leg gets the in-game REVEAL it was waiting for and our own turn loop completes normally. Measured at HEAD verbatim: TURN LOOP LEG: queued=2 -> returned type='reveal' verdict=None queue_left=0; AUDIT: our push landed? 1 call(s) = ['receive_final_reveal']; AUDIT OUTCOME: <Outcome.TECHNICAL_LOSS: 'technical_loss'>; records technical_win{reason: 'opponent_unresponsive', retries_attempted: 2} then game_over{outcome: 'technical_loss'}, and NO audit_verdict ever written. queue_left=0 is the whole defect in one number -- the peer's FINAL_REVEAL was consumed off our own queue and destroyed by the tolerated-jitter drop -- and 1 call(s) is the other half, our push LANDED, so agent_audit_wiring.py:97 took the accusatory branch against a peer that DEMONSTRABLY DID PUSH. THE PREMISE BROKE, NOT THE INFERENCE: :90-93's reasoning that a landed push proves the silence is the peer's own act is SOUND, and not one character of it was edited. Task 2 (28adc4d, fix): next_protocol_message RECORDS every FINAL_REVEAL into ctx.commit_state.early_final_reveal and STILL RETURNS it, while receive_final_reveal consults that buffer at the top of every iteration BEFORE the wait. The asymmetry with HINT is the design: a HINT is buffered and SKIPPED because no caller wants one, a FINAL_REVEAL is buffered and RETURNED because exactly one caller does, and skipping it would starve that caller of the envelope it waits for -- the same false accusation through a different door. The four legs' drop policy is BYTE-UNCHANGED; what changed is that every drop is now free. TWO 150-LINE SPLITS, never compressions, both at seams already named on paper: next_protocol_message -> turn_commit_pull.py (deferred item #11's OWN wording, legs=policy away from primitive=mechanism; turn_commit_wait.py 145 -> 135, moved body BYTE-IDENTICAL by diff bar the two added lines), and the buffer -> final_reveal_buffer.py (turn_buffer.py had 11 lines of headroom; 139 -> 142). Both re-exported, every importer unchanged. Task 3 (0a896aa, test): six cases over two files (split at the gate along the accusation-vs-mechanism seam, 133 + 111, plus _early_reveal_fixtures.py 111), and SEVEN PROBES EVERY ONE WITH A REAL COUNT, run over the two new files PLUS 05-15's four receive cases: P1 the primitive's routing reverted 8 FAILED/2 passed; P2 THE WRONG FIX -- widen the receive ladder instead of routing -- ran 21 ATTEMPTS (10x the shipped 4) and produced a BYTE-IDENTICAL accusation (TECHNICAL_LOSS, 1 technical_win), because the envelope was already eaten and no amount of waiting un-eats it; P3 the audit stops consulting the buffer 4 failed/6 passed; P4 first-wins -> last-wins 1 failed/9 passed; P5 THE DEFENSIVE WRONG FIX (buffer consulted AFTER the ladder) 1 failed/9 passed on 2 != 0 QUEUE PULLS -- it returns the same records and accuses nobody while costing the audit 135 s against a 60 s watchdog_threshold, and counting pulls is the only thing that can see it; P6 take -> peek 1 failed/9 passed. THE COUNTER-CONTROL IS THE POINT: a genuinely silent peer whose full turn we watched still earns TECHNICAL_LOSS + technical_win{opponent_unresponsive}, and it passed UNCHANGED in all six probes -- rule 36 is byte-intact, and a fix that spares the silent peer would have broken the game. SELF-AUDIT CAUGHT A JUSTIFICATION OF MY OWN THAT WAS FALSE: take_final_reveal's first docstring claimed a peek 'would spin forever' -- it would not, the caller returns on the first non-None -- replaced with the reason P6 makes testable and RECORDED as a correction rather than quietly rewritten. THE PLAN UNDER-COUNTED THE FALSE DOCSTRINGS BY ONE (four, not three: next_protocol_message's own 'the caller's own concern to drop' is the sentence the fix falsifies) and its single test file would have breached the gate at ~166 lines. A THIRD FALSE-ACCUSATION DOOR WAS FOUND BY GREPPING PRODUCTION CALLERS, NOT BY READING THE DIFF: turn_commit.py:103, the POLICE branch, calls the pull primitive BARE with no type test -- an early FINAL_REVEAL there becomes Outcome.TECHNICAL_LOSS with reason 'payload must be a dict, got NoneType'. Measured BYTE-IDENTICAL with this plan's routing reverted, so pre-existing; filed as deferred #18, NOT fixed, and this plan makes it strictly better because the ledger is now safe in the buffer while the turn loop mis-declares. Both of the plan's named non-goals filed WITH measurements: #16 the linger's quiet interval is derived from the wrong clock (quiet interval 5 s, total cap 30 s, worst-case arrival->retry response_timeout + backoff_seconds = 35 s, so the retry lands 5 s after the whole linger returned) and #17 the linger drains through a plain queue.get() so 05-17's buffer does NOT cover it (queue 1 -> 0, buffered None, nothing logged). GATES: ruff 0, line-limit exit 0 TREE-WIDE AND exit 0 AGAIN on the five new files named explicitly (the no-argument form enumerates via git ls-files and would have passed VACUOUSLY over untracked files), no-llm OK, suite 1530 passed / 0 failed / 96.64% against the 1524 / 96.62% baseline (+6, exactly the six added), ALL SIX changed/new modules at 100% (turn_commit_pull 26/26, final_reveal_buffer 11/11, agent_audit_exchange 30/30, turn_commit_wait 49/49, commit_state 27/27, turn_buffer 58/58), GATE-6 all three PASS with the evidence RESTORED not committed (2 mtime lines + 1 generated_at + 05-15's two pre-existing game_over counters; every verdict field byte-identical), dev_launch.py exit 0 with ONE shared uid a97adb14e48fc621 across both logs and both filenames, both sides audit_verdict matched=true (6 peer + 6 self each), ZERO technical_win and ZERO watchdog_incident. test_late_peer_teardown PASSED throughout -- deferred #4's fix held and nothing here disturbed it. Knowledge graph refreshed: 7934 nodes / 14322 edges / 481 communities (was 7832 / 14141 / 479); graph.html was skipped by the tool's own 5000-node viz ceiling, which is pre-existing and affects no committed artifact. NOTE ON THIS FIELD'S OWN CLOCK: the prior last_updated (22:45:00Z) was hand-entered ahead of real time, so this update's true UTC reads earlier; the commit timestamps in 05-17-SUMMARY.md are authoritative. Nothing weakened, nothing skipped, no --no-verify.
last_updated: "2026-08-17T20:40:00.000Z"
last_activity: 2026-08-17 -- Phase 05 plan 18 executed (05-18: close the fifth instance of the envelope-boundary class, and pin the class; wave 10, depends_on [05-15, 05-17]) -- THE LAST PLAN OF PHASE 5. Three tasks, four atomic commits: d249625 (test, RED reproduction), f099ad2 (fix, the shared leg + the watchdog marking), e01708c (test, the 12-site class guard + deferred #18 closed / #19 #20 opened), f27b17f (test, the self-audit pin). Seven files created, seven modified. FOUR deviations: two blocking line gates taken as SPLITS, one Rule-2 missing watchdog marking, one Rule-1 unpinned guard found by self-audit AFTER the tasks were done. HEADLINE: an honest peer's published ledger became OUR 0-point technical loss with a decoder's error string attached ('payload must be a dict, got NoneType'), because the initiator's own wait was the one wait in the codebase with no type test. BEFORE: TECHNICAL_LOSS + a parse-error reason, queue_left=1. AFTER: outcome=None, zero accusations, audit_verdict matched=true; and when the REVEAL genuinely never comes, the honest MEASURED opponent_unresponsive instead. Both design candidates were RUN. The class is now enumerated FROM SOURCE across 12 pull sites x 9 MessageType, and it found INSTANCE SIX on its first run (deferred #19, recorded not fixed). Rule 36 intact in three modules. Suite 1539 passed / 96.64% (baseline 1530 / 96.64%); dev_launch exit 0, one shared uid afa6aa3840d63e88, both sides matched, zero technical_win and zero watchdog_incident.
prior_last_activity: 2026-08-17 -- Phase 05 plan 17 executed (05-17: an early peer FINAL_REVEAL is buffered instead of eaten; wave 9, depends_on [05-13, 05-15, 05-16]) -- ORCHESTRATOR-WRITTEN GAP-CLOSURE PLAN. Three tasks, four atomic commits: b29a098 (test, RED reproduction), 28adc4d (fix, the routing), 0a896aa (test, six cases and seven probes), 0cb9aea (docs, deferred #16/#17/#18). Five files created, five modified. FOUR deviations auto-fixed (2 blocking line gates, 1 mis-count of the false docstrings, 1 pre-existing defect found by caller-grep and deliberately filed rather than fixed). HEADLINE: our own turn loop consumed the peer's published ledger as tolerated jitter, after which our audit waited out a ladder for a message that had already been delivered and destroyed and declared OPPONENT_UNRESPONSIVE against a peer that demonstrably did push. BEFORE: queue_left=0, push landed, TECHNICAL_LOSS + technical_win{opponent_unresponsive}, no audit_verdict. AFTER: audit_verdict matched=true over the peer's real commit_pack hashes, zero technical_win. The tempting wrong fix (widen the ladder) was RUN: 21 attempts, byte-identical accusation. The defensive wrong fix (check the buffer after the ladder) was RUN: refuted only by counting queue pulls, 2 != 0. The silent-peer counter-control passed unchanged in all six probes -- rule 36 intact. Suite 1530 passed / 96.64% (baseline 1524 / 96.62%), all six changed modules at 100%; GATE-6 all three PASS; dev_launch exit 0, one shared uid a97adb14e48fc621, both sides matched, zero technical_win and zero watchdog_incident. Knowledge graph refreshed: 7934 nodes / 14322 edges / 481 communities.
progress:
  total_phases: 8
  completed_phases: 4
  total_plans: 64
...
**Current focus:** Phase 05 **GATE MET, verified 2026-08-16** — criterion 2 closed by remote-round attempt 4 (two machines, two networks, agreeing verdicts and matched audits on BOTH sides, live `claude-haiku-4-5`). `/gsd:verify-work 5` then ran a 12-agent adversarial audit and found **five NEW gaps G6–G10** (three blocker-class, none a §10.4 criterion) — recorded in `05-UAT.md` Round 2, planned as 05-12..05-15. **05-12 EXECUTED 2026-08-16 — G9 and G7 closed**, and the corridor sweep found two MORE digest escapes than the plan named (instance 9). **05-13 EXECUTED 2026-08-16 — G6 closed**: the final audit now touches the freeze watchdog once per BOUNDED attempt on both legs, so a 135 s ladder against a stalled tunnel edge ends in a recorded `audit_verdict` instead of `os._exit(1)` at t=60 s — and the plan's own two must_haves disagreed about the receive leg, which is the finding. **05-14 EXECUTED 2026-08-16 — G8 closed**: one inbound hint is decoded at most once and both `take_my_turn` branches stamp the turn actually played — and the plan's own fix was HALF RIGHT, closing the replay for the responder while the initiator still decoded a re-send twice. **05-15 EXECUTED 2026-08-16 — G10 closed, and it is the LAST plan of the phase**: the dead `declare_truthfully` is gone on recorded grep evidence with the rules-15/16/21/22 text quoted in its place, `PRD_mcp_transport.md:65` carries a dated superseded-by note, and the cop now SENDS its rule-21 Capture Claim on the `GAME_OVER` envelope that has existed since Phase 2 — and building that de-risk is what exposed a **pre-existing false-accusation path**: `receive_final_reveal` read `records` off whatever arrived first, so a stray envelope gave `records=[]` / `verdict=None` and lost the game to an honest peer (rules 16/22). **05-08 executed 2026-08-16** (runbook + gate-record correction; its round was already closed by attempt 4 and was not re-run). **All 15 plans are now executed; `/gsd:verify-work 5` is next.** Phase 07 waits.
...
Phase: 05 (cloud-exposure-and-tunneling) — **GATE-5 MET; follow-up gaps open.**
  ...
  **05-13 EXECUTED 2026-08-16** (`d5f2f36`, `a2f7034`, `6920d4d`, `ce89f02`, plus the
  three self-audit follow-ons `2633df4`, `da58bc2` and `63c4ba1`) — see
  `.planning/phases/05-cloud-exposure-and-tunneling/05-13-SUMMARY.md` and the
  frontmatter `stopped_at`/`last_activity` above for the task-by-task account and
  all four revert probes verbatim. **G6 CLOSED.** Headline: `run_final_audit`
  touches the freeze watchdog once per BOUNDED attempt on BOTH legs, so the
  135 s ladder (`4x30 + 3x5`, config/*/network.json) no longer runs unmarked
  against a 60 s `watchdog_threshold` whose freeze action is `os._exit(1)`.
  Measured: the send ladder runs **4 attempts / 140.0 s on an injected clock in
  0.000 s of real time** and still reaches `audit_incomplete` + `audit_verdict`
  with zero `technical_win`; both legs failing costs 280.0 s injected and yields
  exactly `[own_final_reveal_send_failed, own_final_reveal_receive_failed]` with
  the board outcome standing. **NET-07 IS PRESERVED, NOT TRADED AWAY**, and it is
  proven by test rather than asserted in prose: the touch marks an attempt
  STARTING, so the widest gap between two touches is
  `response_timeout + backoff_seconds` = 35 s < 60 s while a WEDGED event loop
  stops producing attempts and is still killed (`["freeze", "exit"]`, in that
  order). Revert probe **P4 is the tempting WRONG FIX itself** — touches removed
  plus `ctx.watchdog.stop()` across the whole audit — and it fails 3 tests; that
  is the discrimination this plan owed. **THE PLAN'S OWN TWO must_haves
  DISAGREED** about the receive leg: its artifact line specified an
  UNCONDITIONAL relaxation while its truth 4 requires the withheld-nonces
  sanction to keep firing, and that control IS a failed receive with a board
  outcome standing. Resolved on `send_verdict is not None` — a push that LANDED
  proves the peer's server accepted our envelope, so rule 36 is byte-unchanged;
  only a DOUBLE own-leg failure is non-accusatory, and it RETURNS rather than
  falling through (with `peer_records == []` the same TECHNICAL_LOSS re-enters
  through the `AUDIT_HASH_MISMATCH` door). P4 also exposed a hole in the plan's
  OWN control: `ArmedWatchdog` drove `Watchdog.check_once()`, which never
  consults the stop flag, so `test_the_audit_never_disarms_the_watchdog` would
  have PASSED against exactly the wrong fix it exists to refute — found by
  RUNNING the wrong fix, not by reading the code. One 150-line SPLIT **at exactly
  150/150**, never a compression (`agent_audit_wiring.py` → `agent_step0_wiring.py`,
  the 05-10 `security/audit.py` precedent). `test_audit_send_failure.py`'s cases
  1–4 are BYTE-UNEDITED, so all three paired fairness controls pass without a
  character changed. Suite **1484 passed / 0 failed / 96.58%** (baseline 1479 /
  96.57%), all five changed src modules at 100%; GATE-6 all three PASS;
  `dev_launch.py` exit 0 with one shared uid `59dc79e25016a0d8`, both sides
  `audit_verdict matched=true`, zero `technical_win` and zero
  `watchdog_incident`.
  **05-14 EXECUTED 2026-08-16** (`f6bbc0f`, `91dbc86`, `54619a1`, `914662c`, plus
  the three follow-ons `9852b3f`, `5371d0d` and `f108a8b`) — see
  `.planning/phases/05-cloud-exposure-and-tunneling/05-14-SUMMARY.md` and the
  frontmatter `stopped_at`/`last_activity` above for the task-by-task account
  and all five revert probes verbatim. **G8 CLOSED.** Headline: **the plan's own
  fix was HALF RIGHT, and the probe is what said so.** It specified repurposing
  `ctx.pending_hints` as a consumed marker written at ARRIVAL, on the ground that
  `maybe_resolve`'s once-per-resolved-turn clear is "precisely the right
  lifetime". Measured, that lifetime is right for the RESPONDER and destroyed
  before the INITIATOR's decode — the two roles' timings differ, and the
  attempt-4 logs are what say so: machine B's six inbound hint records sit at
  `record_turn == envelope_turn + 1` (the hint arrives AFTER its receiver has
  resolved) while machine A's five sit at **delta 0** (it arrives DURING the
  turn, the resolve clears the marker, and `take_my_turn` decodes it a turn
  later). Probed directly, the initiator read **`decodes=2`** against the
  responder's `decodes=1`. Closed by `turn_hint_store.consume_hint`, which writes
  the marker AT CONSUMPTION — the same pop, plus the mark — so the one place that
  SPENDS a hint is the one place that marks it spent; after, both roles read
  `decodes=1`. **THE WINDOW WIDTH IS BYTE-UNCHANGED**, which is the whole
  discipline: marker and window DOVETAIL rather than overlap, and revert probe
  **P4 — the tempting wrong fix of re-narrowing to the pre-05-06
  `turn < ctx.state.turn` — fails 6 tests**, two of them byte-unedited. Only a
  STRICTLY NEWER stamp re-enters (`buffer_if_not_older`'s inclusive `>=` is right
  for an overwrite and wrong for a replay guard; P3, 3 fail), and an arrival with
  no usable stamp cannot prove it is newer either. `ctx.pending_hints` —
  declared, cleared and written since Phase 4 with ZERO production readers —
  finally has one, so `test_turn_buffer.py`'s four assertions on it stop being a
  write-only-buffer trap **without a character changed**. Second defect:
  `turn_actions.py:128` stamped `ctx.state.turn` on the initiator branch on a
  PRECONDITION (commit-reveal ON) the comment ASSUMED rather than stated; with
  `commit_reveal=False` BOTH sides take that branch and the second mover's hint
  went out ONE TURN IN THE FUTURE, corrupting wire evidence *silently* because no
  drop guard fires for a future stamp. Measured on a full toggle-off game: second
  mover hints **[1..15] before, [0..14] after**, first mover [0..15] unchanged as
  the control. **SELF-AUDIT FOUND A TEST THAT FAILED TO FAIL** (`9852b3f`): the
  wire-record case asserted only `got == sent[:len(got)]`, which holds just as
  well when BOTH sides' numbers are in the future, so it PASSED against probe P5
  — the exact regression it exists to catch. Found by RUNNING the wrong fix, the
  same lesson 05-13 recorded. The confounded `token_spend.calls = 0` sentence is
  **deleted**, replaced by two unconfounded proofs, and `_HINT_LOOKBACK_TURNS` is
  now recorded in `docs/PARAMETERS.md` under a new **"Derived protocol constants
  — not Appendix F values"** section with no Status column, deliberately treated
  DIFFERENTLY from 05-12's structural `_MAX_PEER_GAME_ID` with the reason stated
  in both places. One 150-line SPLIT, never a compression
  (`turn_hint_buffer.py` 141 → `turn_hint_store.py`). Suite **1491 passed / 0
  failed / 96.59%** (baseline 1484 / 96.58%), all three changed hint modules at
  100%; GATE-6 all three PASS; `dev_launch.py` exit 0 with one shared uid
  `83143c7b54bdbfbc`, both sides `audit_verdict matched=true`, zero
  `technical_win` and zero `watchdog_incident`.
  **05-15 EXECUTED 2026-08-16** (`4acee3b`, `1c90869`, `0930aac`, `886772d`, plus
  `f80e81b`) — see
  `.planning/phases/05-cloud-exposure-and-tunneling/05-15-SUMMARY.md` and the
  frontmatter `stopped_at`/`last_activity` above for the task-by-task account
  and all eleven probes verbatim. **G10 CLOSED — the last plan of the phase.**
  Headline: **building the thing the plan called a "de-risk" is what exposed a
  PRE-EXISTING false-accusation path.** `receive_final_reveal` read `records`
  off whatever `next_protocol_message` returned FIRST, so ANY envelope ahead of
  the peer's FINAL_REVEAL produced `records=[]` with `verdict=None` —
  byte-indistinguishable from the `{"records": []}` rule-36 evasion
  `security/audit.py` exists to punish — after which every played turn fails
  `audit_state` as "absent from final reveal" and an **honest peer is declared a
  technical loss** (rules 16/22). Reachable BEFORE this plan added a sender:
  `game_over` is a registered tool on our PUBLISHED league surface. Measured
  pre-fix on `[GAME_OVER, FINAL_REVEAL]`: `records=[]`, `verdict=None`, the
  peer's real ledger discarded. Second finding: **the plan's own PREFERRED
  removal of the BARRIER/CAPTURE template rows is wrong** — probe X measures
  **4 failures**, the decisive one a `KeyError` escaping `bluff.compose()`,
  whose entire contract is having no failure mode, because
  `DeceptionPlan(intent=TRUTH, kind=BARRIER)` stays constructible and `BANK`
  must stay TOTAL. The plan's second sanctioned option was taken instead:
  kept, and documented as reserved-and-total-by-design. `declare_truthfully`
  itself IS dead and IS deleted, on grep evidence recorded before anything
  moved — and the plan under-counted its call-site test files by two, which
  probe **P1** measures as **5 collection ERRORS** with both directories
  interrupted and zero tests run. **SELF-AUDIT found a shadowed assertion in
  this plan's own control**: `assert records ==` fires before
  `assert verdict is None`, so both wrong fixes produced the IDENTICAL failure
  line and the verdict half could not fail; re-pinned as one tuple and refuted
  by a purpose-built DEFENSIVE wrong fix (probe **P8** — return the peer's
  ledger correctly but still accuse), found by RUNNING it. Two 150-line
  SPLITS, never compressions (`agent_audit_exchange.py` 153 →
  `agent_audit_observed.py`, `observed()` byte-identical;
  `test_agent_audit_exchange.py` 152 → `test_agent_audit_receive.py`, every
  surviving body byte-unedited). Suite **1507 passed / 0 failed / 96.62%**
  (baseline 1491 / 96.59%), all four new/changed network modules at 100%;
  GATE-6 all three PASS with exactly two new evidence counters —
  `"game_over": 1` under `police_sent` and under `thief_received`, nowhere
  else; `dev_launch.py` exit 0 with one shared uid `6ee059af51c508f5`, the
  claim **sent by the cop and received by the thief at turn 6 with
  byte-identical payloads**, both sides `audit_verdict matched=true`, zero
  `technical_win` and zero `watchdog_incident`.
  **05-17 EXECUTED 2026-08-17** (`b29a098`, `28adc4d`, `0a896aa`, `0cb9aea`) — see
  `.planning/phases/05-cloud-exposure-and-tunneling/05-17-SUMMARY.md` and the
  frontmatter `stopped_at`/`last_activity` above for the task-by-task account and
  all seven probes verbatim. **A rules-16/22 FALSE-DECLARATION PATH REACHABLE BY AN
  HONEST OPPONENT DOING NOTHING WRONG IS CLOSED — by ROUTING, with zero numeric
  values touched.** Headline: **we manufactured the silence we then punished.** A
  peer that finished its turn loop first and pushed its ledger had that envelope
  consumed off our own queue and dropped as tolerated jitter by whichever
  `wait_for_*` leg was running; our audit then waited out its whole ladder for a
  message already delivered and destroyed and — because our OWN push landed —
  declared `OPPONENT_UNRESPONSIVE` at `agent_audit_wiring.py:97`. Measured at HEAD:
  `queue_left=0`, push landed (`1 call(s)`), `TECHNICAL_LOSS` +
  `technical_win{opponent_unresponsive}`, **no `audit_verdict` ever written**.
  **The PREMISE broke, not the inference** — `:90-93`'s "a push that landed proves
  the silence is theirs" is sound and was not edited by one character. Fixed by
  buffering: `next_protocol_message` records every FINAL_REVEAL into
  `ctx.commit_state.early_final_reveal` and **still returns it** (a HINT is
  buffered-and-SKIPPED because no caller wants one; this is
  buffered-and-RETURNED because exactly one caller does), and
  `receive_final_reveal` consults that buffer **before** the wait, so an early
  ledger short-circuits the ladder instead of timing out behind it. **RULE 36 IS
  BYTE-INTACT AND PROVEN SO**: the silent-peer counter-control — a peer whose full
  turn we watched and which then published nothing — still earns
  `TECHNICAL_LOSS` + `technical_win{opponent_unresponsive}`, and it passed
  UNCHANGED in all six revert probes. **BOTH tempting wrong fixes were RUN, not
  argued**: widening the receive ladder gave **21 attempts** (10x the shipped 4)
  and a **byte-identical** accusation, because the envelope was already eaten; the
  *defensive* variant (consult the buffer AFTER the ladder) returns the same
  records and accuses nobody, and is refuted **only** by counting queue pulls
  (`2 != 0`) — 135 s against a 60 s `watchdog_threshold` is what it costs.
  **SELF-AUDIT caught a justification of MY OWN that was false** (`take_final_reveal`
  claimed a peek "would spin forever"; it would not) — replaced with the reason
  probe P6 makes testable, recorded as a correction. Two 150-line SPLITS at seams
  deferred #11 had already NAMED (`turn_commit_pull.py`, body byte-identical bar
  two lines; `final_reveal_buffer.py`). **A THIRD false-accusation door was found by
  GREPPING PRODUCTION CALLERS** — `turn_commit.py:103`, the police branch, pulls
  bare with no type test and turns an early FINAL_REVEAL into
  `TECHNICAL_LOSS`/`'payload must be a dict, got NoneType'`; measured
  byte-identical with the routing reverted, so pre-existing, filed as **#18** and
  NOT fixed. Suite **1530 passed / 0 failed / 96.64%** (baseline 1524 / 96.62%),
  all six changed/new modules at 100%; GATE-6 all three PASS; `dev_launch.py` exit
  0 with one shared uid `a97adb14e48fc621`, both sides `audit_verdict matched=true`,
  zero `technical_win` and zero `watchdog_incident`. Knowledge graph refreshed:
  7934 nodes / 14322 edges / 481 communities.
  **05-18 EXECUTED 2026-08-17** (`d249625`, `f099ad2`, `e01708c`, `f27b17f`) — see
  `.planning/phases/05-cloud-exposure-and-tunneling/05-18-SUMMARY.md` and the
  frontmatter `stopped_at`/`last_activity` above for the task-by-task account and
  all six probes verbatim. **THE LAST PLAN OF PHASE 5**, and the one that stops the
  envelope-boundary class regenerating. **Deferred #18 CLOSED**: the initiator's own
  wait (`turn_commit.py:103`) was the ONE wait in the codebase with no type test —
  it called the pull primitive bare and handed an honest peer's published ledger to
  move handling, where `decode_revealed_action` wants a `move` sub-key and the
  payload is `{"records": [...]}`. Measured at HEAD: `TECHNICAL_LOSS` +
  `technical_win{'payload must be a dict, got NoneType'}`, `queue left = 1` — a
  **false declaration under rules 16/22** and a 0-point loss of a game we did not
  lose. 05-17's buffer already kept `audit_verdict matched=true`, so this was never
  about the audit. **BOTH DESIGN CANDIDATES WERE RUN, not argued**: keep waiting →
  `outcome=None`, zero accusations; end the game → either an `AttributeError` in the
  caller or a **fabricated** `TechnicalWin`, whose own docstring says every field is
  MEASURED by the ladder and *"never assumed or defaulted, so the declaration this
  carries is defensible at audit"*. Both roles now share **ONE leg** —
  `wait_for_reveal_capturing_early_ack`, `h_commit: str | None` — because every
  instance of this class has been a boundary where one path had the discipline and
  its sibling did not, so the repair **removes the second path**. Zero numeric values
  moved. **THE CLASS IS NOW PINNED**: `_pull_site_discovery.py` reads **12 pull
  sites** out of `src/pursuit/network/` by a two-stage AST walk (bounded by the
  return annotation, so `await_opponent_turn` is in and `initiate` is out) and holds
  each to one invariant over **12 × 9 = 108 combinations** — no traceback, named
  `TechnicalWinReason` members only, the ledger never destroyed. All five historical
  instances sit inside that set. **IT FOUND INSTANCE SIX ON ITS FIRST RUN**:
  `turn_buffer.await_move` has no type test at all, so on the `commit_reveal=False`
  path 8 of 9 `MessageType`s return `'payload has neither direction nor x/y keys'`
  (ON is 0 of 9) — filed as **#19**, recorded not fixed, with a test that asserts the
  SCOPE and fails deliberately when #19 is closed. **RULE 36 INTACT**, asserted in
  three modules and passing at HEAD *before* the fix as well as after. **SELF-AUDIT
  caught a guard of my own that no test defended** — `h_commit is not None` could be
  deleted with all 1538 tests green (the `dict.get` → `None == None` trap) — and an
  assertion of mine that could not fail, both now pinned/replaced. Three 150-line
  SPLITS, never compressions. Suite **1539 passed / 0 failed / 96.64%** (baseline
  1530 / 96.64%, +9 exactly); `dev_launch.py` exit 0, one shared uid
  `afa6aa3840d63e88`, both sides `audit_verdict matched=true` over 5 peer turns, zero
  `technical_win` and zero `watchdog_incident`.
  NEXT: `/gsd:verify-work 5` — **all 18 plans are executed** (05-01..05-18).
  **FOURTEEN DEFERRED ITEMS OPEN** — **#4, #10 and #18 are now CLOSED** (the
  late-peer control got a deterministic happens-before gate on 2026-08-17; the turn
  loop's watchdog exposure closed by 05-16; the initiator's bare pull closed by
  05-18) and **#19, #20 are NEW from 05-18**: **#19** `turn_buffer.await_move` has no
  type test, so the `commit_reveal=False` path reads any envelope as a move (8 of 9
  types, `'payload has neither direction nor x/y keys'`; ON is 0 of 9) — instance six
  of the class, latent on a non-shipped toggle, `turn_buffer.py` at 146/150 needs its
  own split; **#20** three modules within two lines of the gate with their seams
  named (`turn_commit.py` 149, `_pull_site_drivers.py` 136, `turn_buffer.py` 146).
  **#16 and #17 remain OPEN, untouched, with their 05-17 measurements**: **#16** the
  linger's quiet interval is derived from the wrong clock (5 s quiet, 30 s cap, 35 s
  worst-case arrival→retry — the retry lands after the linger has returned); **#17**
  the linger drains through a plain `queue.get()`, so 05-17's buffer does NOT cover
  it (`queue 1 → 0`, `buffered None`, nothing logged) — now carried into 05-18's
  invariant as a NAMED exemption, asserted to be a real discovered site and probed to
  confirm the check it exempts is live. Also open: (**#14** (05-15) each dropped envelope costs the
  final-reveal receive leg a whole `(retry_count+1) x response_timeout` ladder now
  that the leg loops — bounded in practice (an honest peer sends at most one
  declaration), identical in shape to `turn_commit_wait.py`'s four `wait_for_*`
  legs, and it must **NOT** be closed by widening `watchdog_threshold`, the same
  reason #10 says so; **#15** (05-15) `tests/unit/services/test_bluff.py` is at
  147/150 with a named seam (move the shared fakes to `_bluff_fixtures.py`, the
  `_hint_decode_fixtures.py` precedent); **#13** (05-14) on the `commit_reveal=False` path
  the second mover's MOVE envelope is stamped `1..16` for turns `0..15` —
  `send_move_only` reads `ctx.state.turn` after the same `maybe_resolve` — the
  same defect class one line from the one just fixed, deliberately NOT closed
  here because the repair changes `turn_commit.initiate`'s signature and on the
  commit-reveal-ON path that `turn` feeds the D-59 hash input and the D-64 ledger
  join key, where a wrong number is a rules-19/22 technical loss rather than an
  evidence blemish; #2, #3, #4, #5 pre-existing; **#8** the ROADMAP progress
  ...
  it **passed in the 05-13 full-suite run** as it did in all three of 05-12's, and was not
  touched. **#10** (05-13) the TURN LOOP carries the IDENTICAL 135 s-vs-60 s exposure G6
  was about — `next_protocol_message`'s `on_attempt` defaults to `None` deliberately, so
  every turn-loop wait leg is byte-identical; which of the two bounds should win MID-GAME
  is a parameter-and-policy decision needing its own plan, and it must **NOT** be closed by
  widening `watchdog_threshold`. **#11** (05-13) four files are within six lines of the
  150-line gate (`turn_commit_wait.py` 145, `test_audit_send_failure.py` 148,
  `test_audit_watchdog.py` 146, `_fakes_agent.py` 144), each with a named seam recorded.
  **#12** (05-13) `call_with_retry`'s injected `sleep`/`clock` seams are not threaded from
  `AgentContext`, so no test exercises the real `backoff_seconds` path at production values).
