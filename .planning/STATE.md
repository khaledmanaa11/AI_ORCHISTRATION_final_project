...
status: executing
stopped_at: PHASE 5 PLAN 14 EXECUTED (2026-08-16) -- G8 CLOSED, AND THE PLAN'S OWN FIX WAS HALF RIGHT, WHICH THE PROBE IS WHAT SAID SO. Task 1 (f6bbc0f, fix): the single-decode guarantee. MEASURED BEFORE at 4fd1d74: `decode_turn_hint` POPS `ctx.incoming_hints` (turn_language_io.py:59) while `_buffer_if_not_older` compares an arrival only against what is STILL buffered, so a hint re-sent for turn N-1 after a pop sailed through the correctly widened window and was decoded a SECOND time, re-driving `observe_reliability` and the belief update on evidence already counted -- 05-06's widening opened it, the old `turn < ctx.state.turn` guard having dropped the duplicate as a side effect. The plan's fix was to repurpose `ctx.pending_hints` as a marker written at ARRIVAL, relying on `maybe_resolve`'s clear as 'precisely the right lifetime'. PROBED DIRECTLY, THAT IS HALF RIGHT: it survives to the RESPONDER's decode (attempt-4 machine B, 6 of 6 inbound records at `record_turn == envelope_turn + 1`, re-measured this session from the committed logs) and is DESTROYED before the INITIATOR's (machine A, 5 of 5 at delta 0 -- the hint arrives DURING the turn, the resolve clears the marker, and `take_my_turn` decodes it only a turn later), leaving the initiator at `decodes=2, second outcome no_evidence` against the responder's `decodes=1, no_hint`. Task 1's follow-on (54619a1, fix): `turn_hint_store.consume_hint` writes the marker AT CONSUMPTION -- the same pop, plus the mark -- so the one place that SPENDS a hint is the one place that marks it spent; after, BOTH roles read `decodes=1`. THE WINDOW WIDTH IS BYTE-UNCHANGED and that is the whole discipline: marker and window DOVETAIL rather than overlap (inside a joint turn the marker refuses the repeat; across the resolve that clears the marker our own turn counter has advanced past the stamp, so the one-turn window refuses it instead). Only a STRICTLY NEWER stamp re-enters -- `buffer_if_not_older`'s inclusive `>=` is right for an overwrite and WRONG for a replay guard, and reusing it unexamined re-admits the exact repeat (probe P3, 3 fail). An arrival with no usable stamp cannot prove it is newer either, so it does not re-enter. `ctx.pending_hints` -- declared at agent_context.py:112, cleared at turn_resolve.py:96 and written since Phase 4 with ZERO production readers -- now has one, so test_turn_buffer.py's four assertions on it stop being a write-only-buffer trap WITHOUT A CHARACTER CHANGED. Task 2 (91dbc86, fix): `turn_actions.py:128` stamped `ctx.state.turn` on the initiator branch, justified by a comment claiming the initiator's own `maybe_resolve` is a no-op -- a PRECONDITION (commit-reveal ON) the comment ASSUMED rather than stated. With `ctx.security.commit_reveal` false (turn_commit.py:63-64,:100-101, a supported path pinned by test_commit_reveal_protocol.py) `pending_action` is never set, BOTH sides take that branch, the second mover's `maybe_resolve` advances N->N+1, and the hint went out ONE TURN IN THE FUTURE -- the original G4 defect, still live there, and there it corrupts wire evidence SILENTLY because no receiver's drop guard fires for a future stamp (rule 20). Measured on a full toggle-off game: second mover hints [1..15] BEFORE, [0..14] AFTER, with the first mover's [0..15] unchanged as the control. Latent, not active -- shipped config is `commit_reveal: true` -- and fixed anyway. Task 3 (914662c, test): 7 tests in three modules. The single-decode cases assert at the DECODE boundary, not at the buffer, because the defect is not 'a dict got overwritten' but the belief update driven twice: they count `decode_incoming` and `observe_reliability` calls and read `decode_turn_hint`'s own outcome word, whose two values say the right things apart (`no_evidence` = a text ARRIVED and was decoded, `no_hint` = nothing was in the buffer). BOTH ROLES' TIMINGS ARE COVERED SEPARATELY, which is the point. The toggle-off case's GROUND TRUTH IS DERIVED (0..N-1) AND DELIBERATELY NOT THE MOVE ENVELOPE, which is itself stamped 1..16 on that path -- comparing hints against moves would have compared two wrong numbers and passed vacuously. FIVE REVERT PROBES: P1 guard removed 3 fail, P2 THE PLAN'S OWN ARRIVAL-TIME MARKER 1 fail (exactly the initiator case, the responder case passing -- which IS the measurement), P3 the inclusive comparison 3 fail, P4 THE TEMPTING WRONG FIX (re-narrow the window to pre-05-06) 6 FAIL including two BYTE-UNEDITED tests, P5 `ctx.state.turn` restored 2 fail. SELF-AUDIT FOUND A TEST THAT FAILED TO FAIL (9852b3f): `test_the_peer_receives_the_corrected_numbers_on_the_wire` asserted only `got == sent[:len(got)]`, which holds just as well when BOTH sides' numbers are one turn into the future, so it PASSED against probe P5 -- the exact regression it exists to catch. Re-pinned on the played turns; P5 now fails 2 of 2 there. Found by RUNNING the wrong fix, 05-13's lesson again. THE CONFOUNDED `token_spend.calls = 0` SENTENCE IS DELETED, not reworded: that same zero also covers the COMPOSE half, whose texts came from the template bank (services/language_turn.py:76-81), so it is equally consistent with 'no key at all'. Two unconfounded proofs replace it -- (1) SHAPE not statistics, `no_hint` is emitted on exactly one branch, the buffer pop returning nothing, UPSTREAM of every provider concern, while a key-starved decode logs `no_evidence` WITH the text (llm/decode.py:66-67); (2) the 6/6 boundary measurement. `_HINT_LOOKBACK_TURNS` IS NOW RECORDED IN docs/PARAMETERS.md under a NEW 'Derived protocol constants -- not Appendix F values' section with its derivation, both measurements, and NO STATUS COLUMN ON PURPOSE (a derived constant is not fixed/minimum/negotiable), still a source constant with no config leaf (rule 1); both the doc and the source state why it is treated DIFFERENTLY ON PURPOSE from 05-12's `_MAX_PEER_GAME_ID` -- one is a message ORDER two implementations must agree on, the other a structural 255-byte path-component limit nobody should tune, and they are deliberately NOT harmonised. ONE 150-LINE SPLIT, never a compression: turn_hint_buffer.py measured 141/150 with the guard in it and the buffers moved into turn_hint_store.py (88 + 91). ONE DEFERRED ITEM LOGGED AND NOT FIXED (#13): on that same toggle-off path the second mover's MOVE envelope is stamped 1..16 for turns 0..15 (`send_move_only` reads `ctx.state.turn` after the same `maybe_resolve`) -- same defect class one line away, but the repair changes `turn_commit.initiate`'s signature and on the commit-reveal-ON path that `turn` feeds the D-59 hash input and the D-64 ledger join key, where a wrong number is a rules-19/22 technical loss rather than an evidence blemish. GATES: ruff 0, line-limit exit 0, no-llm OK, GATE-6 all three PASS (evidence JSON differed in exactly 3 timestamp lines, every verdict field byte-identical, RESTORED not committed), dev_launch.py exit 0 with ONE shared uid 83143c7b54bdbfbc, both audit_verdict matched=true, ZERO technical_win and ZERO watchdog_incident, police hints [0,1,2,3,4] == its reveals and thief hints [0,1,2,3] == reveals[:-1], both sides decoding real inbound hints. SUITE 1491 passed / 0 FAILED / 96.59% against the 1484 / 96.58% baseline -- +7 tests, +0.01pp, all three changed hint modules at 100%. The first full-suite run hit deferred item #4's documented 0.3 s real-socket teardown flake once; re-run alone 3/3 pass and it did not fire in either subsequent full run. Knowledge graph refreshed: 7656 nodes / 13731 edges / 465 communities (was 7588 / 13614 / 465). Nothing weakened, nothing skipped, no --no-verify.
prior_stopped_at: PHASE 5 PLAN 13 EXECUTED (2026-08-16) -- G6 CLOSED, AND THE PLAN'S OWN TWO must_haves DISAGREED, WHICH IS THE FINDING. MEASURED FIRST, at 448ca09: grepping src/ for the call form watchdog.touch() returned EXACTLY FIVE sites (turn_buffer.py:103,151, turn_commit_send.py:52,132, turn_commit_wait.py:70) -- ALL turn-loop, NONE in the audit path -- while run_agent arms the watchdog at agent_entrypoint.py:76 and stops it only at :134, spanning run_final_audit at :110, and config/*/network.json gives a ladder of 4x30 + 3x5 = 135 s against a 60 s watchdog_threshold whose freeze action is os._exit(1). So against a peer whose socket accepts TCP and never answers -- a stalled tunnel edge, exactly the drop 05-11 exists for -- we died at t=60 s and 05-04's record_audit_incomplete at t~135 s NEVER RAN: log ends on watchdog_incident, no audit_verdict, and the peer then declares US opponent_unresponsive. The 2026-08-13 artifact through a second door, and 05-04's authors KNEW the no-touch fact -- they cite it at agent_entrypoint.py:127-129 as why stop_watchdog must precede the linger. THEY PROTECTED THE LINGER AND STEPPED OVER THE AUDIT. Task 1 (d5f2f36, fix): the touch is now the FIRST statement of push_final_reveal's per-attempt _call closure, and next_protocol_message gained a keyword-only on_attempt (DEFAULT None, so all four turn-loop wait legs are byte-identical) that receive_final_reveal is the only caller to pass. THE PLACEMENT IS THE WHOLE FIX AND NOT A BYPASS: each attempt is itself bounded by response_timeout, so the widest gap between two touches is response_timeout + backoff_seconds = 35 s < 60 s, while a wedged event loop stops producing attempts and is STILL KILLED. Task 2 (a2f7034, fix): the RECEIVE leg stops accusing -- and here the plan contradicted itself. Its artifact line says record_technical_loss is reserved for mismatch / unresolved loop (an UNCONDITIONAL relaxation) while its own truth 4 requires withheld nonces -> TECHNICAL_LOSS to keep firing, and that control IS a failed receive with a board outcome standing. Implemented as written it would have DELETED a fairness control its own must_haves demand -- the receive-leg twin of the wrong fix. RESOLVED ON send_verdict is not None: a push that LANDED proves the peer's server accepted our envelope, so silence afterwards is the peer's own act and rule 36's sanction is BYTE UNCHANGED; only when BOTH of our own legs failed is the evidence about us, and then it is a second non-accusatory audit_incomplete naming OUR OWN receive. RETURNING rather than falling through is load-bearing: with peer_records == [] every honest turn fails at audit_state and the same TECHNICAL_LOSS re-enters through the AUDIT_HASH_MISMATCH door. Task 3 (ce89f02, test): the window was INEXPRESSIBLE before, for two reasons both now removed -- make_ctx installs a FakeWatchdog that checks nothing and forces watchdog_threshold=0, and late_peer_harness.py says in its own docstring that the watchdog is DELIBERATELY not started. New _fakes_watchdog.py runs the REAL Watchdog with clock/on_freeze/exit_action injected and check_once() driven by hand; make_ctx gained a watchdog= seam and now merges net_overrides into ONE dict before dataclasses.replace, which previously made the five fast-test defaults UN-OVERRIDABLE (TypeError). MEASURED: the send ladder runs 4 attempts / 140.0 s on the injected clock in 0.000 s REAL and reaches audit_incomplete + audit_verdict with zero technical_win; both legs failing costs 280.0 s injected and yields exactly [own_final_reveal_send_failed, own_final_reveal_receive_failed] with the board outcome standing. FOUR REVERT PROBES: P1 push touch removed 1 fail (checks=[False,True,True,True]), P2 receive hook removed 1 fail, P3 receive accuses unconditionally 1 fail, and P4 -- THE WRONG FIX ITSELF, touches removed plus ctx.watchdog.stop() across the whole audit -- 3 FAIL, which is the discrimination this plan owed. P4 also exposed a hole in MY OWN control (da58bc2): ArmedWatchdog drove Watchdog.check_once(), which never consults the stop flag (only _run does), so a STOPPED watchdog looked armed and test_the_audit_never_disarms_the_watchdog would have PASSED against exactly the wrong fix it exists to refute -- an unfalsifiable control certifies the thing it forbids. Found by RUNNING the wrong fix, not by reading the code. THREE FALSIFIED CLAIMS corrected in source (2633df4) -- agent_entrypoint.py's teardown comment, test_agent_teardown.py's docstring and late_peer_harness.py's -- with the REASON distinguished from the CONCLUSION, since the linger is a drain loop with no bounded attempt of its own and the ordering stands. Then the correction itself shipped a stale number and was corrected again (63c4ba1): the call-form grep returns SIX sites, not seven, because the receive leg's touch is PASSED not called, so that grep UNDERCOUNTS the audit path by one -- 2633df4's message keeps the wrong number, append-only. ONE 150-LINE SPLIT AT EXACTLY 150/150, never a compression (6920d4d): agent_audit_wiring.py hit the gate exactly, so declare_step0 + write_declaration moved VERBATIM into agent_step0_wiring.py (79) and are re-exported, dropping it to 96 -- 05-10's security/audit.py precedent of splitting AT the gate rather than at the breach. GATES: ruff 0, line-limit exit 0, no-llm OK, GATE-6 all three PASS (evidence JSON differed in exactly 3 timestamp lines, every verdict field byte-identical, RESTORED not committed), dev_launch.py exit 0 with ONE shared uid 59dc79e25016a0d8, both capture, both audit_verdict matched=true, ZERO technical_win and ZERO watchdog_incident. SUITE 1484 passed / 0 FAILED / 96.58% against the 1479 / 96.57% baseline -- +5 tests, +0.01pp, and all five changed src modules at 100%. test_audit_send_failure.py's cases 1-4 are BYTE-UNEDITED, so the three paired fairness controls pass without a character changed. THREE DEFERRED ITEMS LOGGED: #10 the TURN LOOP has the identical 135 s-vs-60 s exposure and was deliberately NOT changed (which bound should win mid-game is a parameter-and-policy decision, and it must NOT be closed by widening watchdog_threshold), #11 four files within six lines of the gate with a named seam each, #12 the injected sleep/clock seams are not threaded from AgentContext. Knowledge graph refreshed: 7588 nodes / 13614 edges / 465 communities (was 7524 / 13490 / 466). Nothing weakened, nothing skipped, no --no-verify -- the pre-commit ruff gate caught a W605 in one docstring and it was reworded, not bypassed.
last_updated: "2026-08-16T20:45:00.000Z"
last_activity: 2026-08-16 -- Phase 05 plan 14 executed (05-14: the hint channel is correct on every supported path; wave 6, depends_on [05-06, 05-12, 05-13]). Three tasks, SEVEN atomic commits: f6bbc0f (fix, the single-decode guarantee), 91dbc86 (fix, both branches stamp the turn actually played), 54619a1 (fix, the marker moved from arrival to consumption + the 150-line split), 914662c (test, 7 cases in three modules), plus 9852b3f (test, a self-audit correction to a case that failed to fail), 5371d0d (docs, deferred item #13) and f108a8b (chore, graph refresh). Five files created, seven modified. FIVE deviations (1 bug -- the plan's own marker placement, 2 blocking, 1 stale cross-reference, 1 scope-boundary deferral). The headline deviation is that THE PLAN'S OWN FIX WAS HALF RIGHT: its arrival-time marker survives to the responder's decode and is destroyed before the initiator's, which still decoded a re-send TWICE -- caught by probe, not by reading, and closed by writing the marker at consumption. Five revert probes recorded verbatim, P2 being the plan's own version (1 fail, exactly the initiator case) and P4 the tempting wrong fix of re-narrowing the window (6 fail). Suite 1491 passed / 0 failed / 96.59% (baseline 1484 / 96.58%); GATE-6 all three PASS; dev_launch exit 0 with one shared uid 83143c7b54bdbfbc, both sides matched, zero technical_win. Knowledge graph refreshed: 7656 nodes / 13731 edges / 465 communities.
prior_last_activity: 2026-08-16 -- Phase 05 plan 13 executed (05-13: the audit survives long enough to be honest; wave 5, depends_on [05-04, 05-12]). Three tasks, SEVEN atomic commits: d5f2f36 (fix, the per-attempt watchdog touch on both legs), a2f7034 (fix, the receive leg stops accusing a peer that may have answered), 6920d4d (refactor, the split at exactly 150/150), ce89f02 (test, the REAL Watchdog on an injected clock + the mirrored receive-leg case), plus three self-audit follow-ons 2633df4 (docs, three claims this fix falsified), da58bc2 (test, a hole in my own NET-07 control found by running the WRONG FIX as a fourth probe) and 63c4ba1 (docs, the stale-grep correction that itself shipped a stale grep count). Three files created, ten modified. SEVEN deviations auto-fixed (4 bug/self-correction, 3 blocking). The headline deviation is that the PLAN CONTRADICTED ITSELF: its artifact line specified an unconditional receive-leg relaxation while its own truth 4 requires the withheld-nonces sanction to keep firing -- resolved on send_verdict is not None, so rule 36 is byte-unchanged and only a DOUBLE own-leg failure is non-accusatory. Four revert probes recorded verbatim, P4 being the tempting wrong fix (disarm the watchdog across the audit), which fails 3 tests. Suite 1484 passed / 0 failed / 96.58% (baseline 1479 / 96.57%); GATE-6 all three PASS; dev_launch exit 0 with one shared uid and both sides matched, zero technical_win, zero watchdog_incident. Knowledge graph refreshed: 7588 nodes / 13614 edges / 465 communities.
progress:
  total_phases: 8
  completed_phases: 4
  total_plans: 62
...
Phase: 05 (cloud-exposure-and-tunneling) — **GATE-5 MET; follow-up gaps open.**
  ...
  **deleted**, replaced by two unconfounded proofs, and `_HINT_LOOKBACK_TURNS` is
  ...
  NEXT: `/gsd:execute-phase 5` for **05-15** — 05-01..05-14 are done.
  **TEN DEFERRED ITEMS OPEN** (**#13** (05-14) on the `commit_reveal=False` path
