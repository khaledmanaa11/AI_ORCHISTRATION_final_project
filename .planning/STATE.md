...
status: executing
stopped_at: PHASE 5 PLAN 07 EXECUTED (2026-08-14) -- G5 CLOSED. A keyless run is now VISIBLE and HONEST, and the fallback itself is byte-for-byte what Phase 4 sanctioned. Task 1 (951d87d): client.has_api_key() -- presence only, bool, beside the ONE definition of the env var name -- and a single _log.warning inside _build_provider's existing branch structure when the resolved provider class is NOT TemplateProvider and no key is present. WARNING is load-bearing and was MEASURED, not assumed: there is no logging.basicConfig anywhere in this project, so the root logger's WARNING default is the only level that reaches stderr, and `env -u ANTHROPIC_API_KEY uv run python -c "_build_provider(...)"` prints the whole line on a bare interpreter. Keyed off the resolved CLASS, never the string "claude_api", so registering another real provider later stays correct for free. bluff.compose's LlmFailure->hint_bank branch got _log.debug, deliberately NOT warning: it fires every turn on a keyless box and would bury the one startup line that matters. Task 2 (50ac2fe): language_wiring.declared_llm_name() replaces declare_step0's pure config echo -- template provider OR no key -> "template-fallback (no LLM calls)", else the configured model_id. MEASURED for all four combinations (claude_api+nokey / claude_api+key / template+key / template+nokey -> fallback, claude-haiku-4-5, fallback, fallback) and confirmed end-to-end by the GATE-6 run, whose two collected declarations now read llm_name: 'template-fallback (no LLM calls)' where the 2026-08-13 machine-B artifact read claude-haiku-4-5 on a box with token_spend.calls=0 on every turn. THE DECLARATION SHAPE IS PINNED: len == 10 and the key set asserted against the ten Sec5.5 names SPELLED OUT IN THE TEST rather than read back from DeclarationField, so a rename or an addition on either side fails -- it rides inside sign_declaration's HMAC and handshake_step0 verifies the digest, which is why GATE-6 was re-run: criterion 3 (step0_verified_before_move_1) PASS, along with 1 and 2. bluff_prompt: "phrasing a claim for a player" -- the exact wording behind machine A's turn-4 "The player is currently positioned..." -- became "You are a player ... stating a claim about yourself", plus a first-person STYLE_GUIDE bullet, with docs/PRD_deception.md Sec6's verbatim quote updated IN THE SAME COMMIT and a new test that parses that fenced block out of the markdown and asserts it EQUALS the shipped string, so the two can never drift silently again. NO post-compose person validator (fuzzy detection would silently divert good hints to the bank -- exactly the failure this plan closes) and NO hard startup failure (that would break a documented Phase-4 acceptance criterion and REMOTE-ROUND-RUNBOOK.md's own instruction that the key is optional). THE FALLBACK IS PROVEN UNCHANGED, not asserted: tests/integration/test_llm_degradation.py's four cases plus test_language_timing.py and test_gate4.py all pass UNEDITED with the key explicitly unset, and the full suite re-run under `env -u ANTHROPIC_API_KEY` gives the identical 1327. THREE DEVIATIONS, all auto-fixed: (1) Rule-3 line gate -- _declared_llm_name in agent_audit_wiring.py took it to 157/150 (the gate counts docstring lines), so the helper MOVED to language_wiring.py as public declared_llm_name(model), where it sits beside _build_provider asking the identical question for a different audience; 134 and 129 after, and that is a STRONGER form of the plan's own "one place decides provider identity" than two sites would have been. (2) Rule-2 -- anthropic_provider.py:101 already carried a SECOND executable copy of the literal "ANTHROPIC_API_KEY" inside its NO_KEY message; _API_KEY_ENV_VAR was promoted to public API_KEY_ENV_VAR and that message now formats it, leaving src/ with exactly ONE executable definition (client.py:25) plus three docstring mentions, and the rendered text byte-identical so its existing test passes unedited. (3) Rule-3 -- two plan-named test paths did not exist; services/llm tests live under tests/unit/services/. Measured: 1327 passed / 96.37% (baseline 1308 / 96.36%, +19 tests, where the plan predicted +6 to +9 -- the extra come from two rule-4 sentinel controls, a four-way template/key parametrisation, the PRD drift guard, and the shape assertion running on EVERY declaration path), ruff 0, line-limit exit 0, no-llm-in-strategy OK, GATE-6 three PASS. ONE HONEST FINDING investigated before committing rather than waved through: gate6_measurement_evidence.json's envelope_counts moved by more than timestamps -- police_received gained hint:4 and thief_received hint:5 where the committed file recorded NO received hints at all. Cause is NOT this plan (which changes one declaration string and two log lines and cannot touch hint delivery): the committed evidence was generated 2026-08-09 under commit 0427137, BEFORE 05-06 fixed the receive-side drop window and the responder's outgoing stamp, and this is the first GATE-6 re-run since, so 05-06's fix is simply appearing in the evidence for the first time (thief_sent 5->4 is 05-06's other half, no hint composed for an already-resolved terminal turn). 04-10-SUMMARY.md also quotes STYLE_GUIDE verbatim and was deliberately LEFT ALONE -- it is a point-in-time record of what plan 04-10 shipped, and rewriting it would falsify history; the live spec is docs/PRD_deception.md, which is what the drift-guard test pins. config/*/games_played.json untracked and never edited (rule 38 -- only real games moved those counters). Knowledge graph refreshed (7205 nodes / 13045 edges / 439 communities); graphify explain declared_llm_name resolves to language_wiring.py L62 with edges to get_provider_class and has_api_key and back from declare_step0. NOTHING TICKED in ROADMAP.md (standing convention); docs/phases/phase-5/TODO.md row 05-07 marked in-progress with both hashes. NEXT -- 05-08 is the ONLY plan left in Phase 5, and it is HUMAN-RUN: the remote round attempt 2 on two machines/networks, the one thing that can close GATE-5 criterion 2. The operator will now see the keyless warning at startup on any machine without a key, and the two machines' declarations will no longer be byte-indistinguishable when one is live and the other is not. PRIOR: PHASE 5 PLAN 09 EXECUTED (2026-08-14) -- DEFERRED ITEM #1 CLOSED, AND THE MEASUREMENT CHANGED THE FIX. A transport-level connection failure is now a bounded, measured, correctly-attributed verdict instead of a process death. The plan's own premise was HALF RIGHT and the harness caught it: httpx.ConnectError is the raw shape only when the fault lands on an ALREADY-OPEN session (client.call_tool -- what 05-04 happened to capture, and what the plan was written from); on the CONNECT path, which is how EVERY outgoing envelope in this codebase starts (async with ctx.runtime.client()), fastmcp 3.4.5 catches it at client/client.py:616-624 and re-raises RuntimeError(f"Client failed to connect: {exc}") from exc, preserving ONLY httpx.HTTPStatusError and McpError unwrapped. So with Task 1 landed and every new test green, late_peer_round(linger=False) STILL ended with the late peer on game_over and nothing after it -- the 2026-08-13 artifact, unfixed (probe D, recorded verbatim). Task 1 (f31ece5): RETRYABLE_TRANSPORT_ERRORS widened to (McpError, DeadlineExpired, httpx.TransportError), NOT httpx.HTTPError -- HTTPStatusError's MRO goes straight to HTTPError and the 403 from 05-02's SharedSecretMiddleware is an application answer about OUR OWN credentials; measured non-vacuous by probe C, where HTTPError swept the production 403 into the ladder and BOTH 403 controls failed. httpx.LocalProtocolError and httpx.UnsupportedProtocol JOIN THE RAISE-FIRST CLAUSE rather than narrowing the tuple, so any future httpx addition under TransportError stays retryable by default; both are OUR OWN deterministic faults, so retrying burns 3 x backoff_seconds and ends in a durable false TechnicalWin against a peer that never received a valid request (UnsupportedProtocol is reachable on league day -- REMOTE-ROUND-RUNBOOK.md has the operator paste PURSUIT_OPPONENT_URL by hand and a missing https:// prefix raises it, measured). ToolError's 06-06 contract untouched and both pre-existing NET-06 test files byte-unmodified (git diff --stat 384da44 HEAD empty). deadline.py SPLIT into deadline_errors.py (relocate-and-re-export, the agent_audit_verdict/game_identity/secret_wiring/turn_hint_buffer precedent) because the taxonomy takes it to ~162 code lines against the 150 gate -- CLAUDE.md says split, never compress; DeadlineExpired identity verified IDENTICAL across modules so no except clause forked. uv add httpx (never pip): src/ imports it now. Task 2 (f602eb3): deadline_errors.unwraps_to_retryable contains fastmcp's wrapper by its DIRECT CAUSE against the same two named tuples -- never by the RuntimeError class, which our own bugs raise and which would be a catch-all in all but name; no cause, an unrelated cause, or a raise-first cause all re-raise untouched, so a WRAPPED scheme-less URL still fails loudly (its own control). error_evidence gives one definition of last_error naming the wrapper AND its cause. Verification item 6 clean: grep for except Exception/BaseException in deadline.py -> no match. THE DECISIVE RE-MEASUREMENT (linger=False, A's listener genuinely closed): the thief's log now ends game_over -> audit_incomplete -> message_received -> audit_verdict{matched:true}, peer_error None, ZERO technical_win on EITHER side, final_a == final_b == CAPTURE -- so the answer to the plan's explicit question is audit_incomplete + AUDIT_VERDICT, not audit_incomplete + technical_win: the fall-through's receive_final_reveal did NOT exhaust, because A had already pushed its own FINAL_REVEAL onto B's queue before tearing down. The receive-side rule-36 sanction is untouched and still pinned. Case 2 reached 05-04's record_audit_incomplete path exactly as predicted with NO change to 05-04's code, and 05-04's pinned no-linger test now passes through its audit_incomplete branch rather than its peer_error branch -- it was written to assert the durable PROPERTY precisely so it would survive this fix, and it did, unedited. Four revert probes recorded verbatim in the SUMMARY: (A) both fix cases fail pre-fix with httpx.ConnectError escaping production code out of turn_commit_send.py:46 and agent_audit_wiring.py:122, controls 3/4/5 already GREEN pre-fix; (B) the tuple-only partial fix makes case 5 fail with "DID NOT RAISE TransportError", so an always-green control is not a vacuous one; (C) HTTPError in place of TransportError fails both 403 controls; (D) the whole plan minus the wrapper fix leaves the artifact intact. FIVE NEW TEST FILES, including tests/integration/test_connect_failure_containment.py -- a REAL fastmcp Client built exactly as PeerRuntime.client() builds it, pointed at a closed loopback port, asserting fastmcp's wrapper explicitly so a version bump that changes it fails loudly instead of silently reopening the crash. Measured: full suite 1308 passed / 96.36% coverage (baseline 1293 / 96.35%, +15 tests), ruff 0, line-limit exit 0, no-llm-in-strategy OK, GATE-6 all three book Sec10.4 criteria PASS (evidence JSON differs only in two predates_detail mtimes, generated_at, and the police/thief hint counts swapping 4<->5 per 05-06's documented best-effort tail; every verdict field byte-identical; restored with git checkout so the diff stays code-only), dev_launch.py exit 0 in 17 s with BOTH sides game_over=capture -> audit_verdict matched=true and zero audit_incomplete/technical_win -- 05-04's measured state not regressed. test_late_peer_teardown.py run explicitly and ALONE: 2 passed, 33.28 s (the no-linger case 21.42 s, up from 14.83 s with Task 1 alone, because the failing push now walks the whole ladder instead of dying on attempt 1) -- deferred item #4's load-sensitive 0.3 s race did NOT reproduce on a quiet box and nothing was relaxed. THREE DEVIATIONS, all auto-fixed: two Rule-3 line-gate SPLITS (deadline_errors.py; test_deadline_httpx.py + test_deadline_wrapped_connect.py, since test_deadline.py at 104 and test_deadline_retry.py at 105 had no room) and ONE RULE-1 fix that is the substantive one -- the planned fix did not fix the defect, and shipping Task 1 alone would have been an overclaim. TWO NEW DEFERRED ITEMS: #5 (EXAMINED AND ACCEPTED, not a defect) an in-game ladder exhaustion accuses the peer even when the fault was OUR OWN uplink/ngrok/DNS -- pre-existing NET-06 policy that McpError has carried since 02-07 and 06-06 affirmed, and strictly better than the pre-fix crash; TunnelManager.ensure_connected named as the one honest narrowing. #6 (LOGGED, NOT FIXED) a 5xx/429 -- ngrok answers 502 when the upstream local server blips -- is still an uncaught httpx.HTTPStatusError mid-game, the same rule-36 crash class through a different door; mcp/client/streamable_http.py calls raise_for_status() at five sites and fastmcp PRESERVES that class unwrapped. Correct for the 403 by this plan's own constraint; branching on HTTP status codes is a policy decision needing its own plan. Knowledge graph refreshed (05-96: 7127 nodes / 12937 edges / 437 communities); graphify explain unwraps_to_retryable resolves to deadline_errors.py L142 with call_with_retry as a PRODUCTION caller. test_transport_failure_containment.py 146/150 and deadline_errors.py 141/150 -- no headroom in either. NOTHING TICKED in ROADMAP.md (standing convention); docs/phases/phase-5/TODO.md gained a 05-09 row marked with both hashes. NEXT -- 05-07 (G5, keyless LLM made legible), then 05-08 (HUMAN remote round attempt 2, the only thing that can close GATE-5 criterion 2); if that round produces a 502 or 429, capture the console output -- it is the missing measurement deferred item #6 needs. PRIOR: PHASE 5 PLAN 06 EXECUTED (2026-08-14) -- G3 AND G4 CLOSED ON THE CODE SIDE, plus G1's 17.4 s stagger half. The language channel delivers in BOTH directions for the first time, and every inbound HINT is finally durable wire evidence. Measured on a real two-process loopback game (dev_launch, exit 0, 17.4 s, both sides game_over=capture -> audit_verdict matched=true): police hints stamped 0,1,2,3,4 and thief hints 0,1,2,3 -- BOTH equal to the turn actually played, so the 2026-08-13 round's 0..4-vs-1..5 asymmetry is gone -- BOTH sides' logs now carry message_received+hint records (police 4, thief 5, where the round had ZERO on either machine), and BOTH sides have at least one non-no_hint incoming_hint (the round gave machine B five no_hint in a row with token_spend.calls=0, and no test in 1262 caught it). Task 1 (7619758): record_hint MOVED to a new src/pursuit/network/turn_hint_buffer.py (turn_buffer.py was 146/150, no room; the same split turn_resolve.py already made out of that file) and re-exported from turn_buffer with an explicit __all__, so all three call sites, turn_commit_wait's own turn_buffer.record_hint(...) reference and every monkeypatch keep resolving; the receipt is logged via turn_commit_send.log_received BEFORE the drop guard (a hint we DISCARD still crossed the wire -- rule 20), with the record's own turn = OUR ctx.state.turn and the nested envelope keeping the peer's declared turn verbatim (the exact 06-05 Gap-1 split, so a hint record can never become an attacker-controllable audit join key), and state_from=state_to=ctx.machine.state because receiving a hint changes no state. The Envelope is rebuilt locally rather than threaded down, which is lossless (every caller has already established the type is HINT) and leaves the three call sites byte-unmodified. Proven inert to the audit by a PAIRED before/after over the same log, not by reading observed(). Task 2 (CONTENT IS IN ead48df -- see the collision note below): the window relaxation and the responder stamp fix landed TOGETHER in ONE commit as the plan required, because uniform correct stamping ALONE takes the round from 3-of-10 decodes to 0-of-10. _HINT_LOOKBACK_TURNS = 1 is a NAMED module constant carrying its derivation in a comment -- a hint for turn N is emitted at the TAIL of turn N, after REVEAL(N) plus an LLM compose, so the receiver has already resolved to N+1 when it arrives; one turn is the MINIMUM that makes the channel deliverable at all, structural like agent_audit_wiring._DECLARE_RETRIES, and explicitly NOT a PARAMETERS.md value and NOT a config key (CLAUDE.md rule 1); a genuinely older hint still drops. The freshness guard the window makes necessary (_usable_stamp/_buffer_if_not_older) keeps the FRESHEST hint per sender and reads the stored stamp DEFENSIVELY: validate_hint_payload runs only inside build_hint on the SEND path and Envelope.from_dict checks nothing but isinstance(payload, dict), so anything that is not a plain int (missing, None, str, bool, float, list, dict -- seven shapes, each tested) means no-usable-stamp -> replace, never a TypeError, which NO call site catches and which would escape run_turn_loop and end the game (the forfeit-caused-by-a-hint failure 04-12's deviation exists to prevent). turn_actions: the responder stamps pending.turn (the turn it actually committed and revealed under) instead of the post-maybe_resolve ctx.state.turn, and NEITHER branch composes for a turn that already resolved -- 17.4 s of the round's 18 s inter-side divergence, measured within machine B's own clock. Both non-vacuity reverts recorded VERBATIM in the SUMMARY, including the one whose result CONTRADICTED the plan's prediction: reverting (a) alone fails assertion 2 exactly as predicted (thief reads no_hint x4), but reverting (c) alone fails assertion 1 ([1,2,3,4] != [0,1,2,3]) and does NOT fail assertion 2 -- and cannot, because 05-UAT.md G4's own finding is that the mis-stamp is the ONLY reason the police decoded anything at all, so reintroducing it cannot stop delivery. Task 3 (f32bb3a): five tests that certified the bugs re-specified, none deleted, none loosened, each stating its reason in a docstring -- test_turn_buffer.py's drop test used ctx.state.turn - 1, which is what a responder hits on EVERY correctly stamped hint, so it moved to a genuinely too-late hint (two turns back) plus a NEW sibling pinning the other edge of the window, and four tests asserting only ctx.pending_hints (ZERO production readers in src/ -- decode pops ctx.incoming_hints) gained the incoming_hints assertion beside the old one. test_gate4.py's len(turns) == ctx.state.turn is now the DERIVED per-role expectation (initiator = turns, responder = turns - 1, with the derivation stated) and the measurement CONFIRMED it exactly -- police 5/5, thief 4/5 -- rather than a number read off a run and frozen, which is how :82 froze the original bug. GATE-4-MEASUREMENT.md criterion 3 amended honestly: the police-side numbers are unchanged (the initiator still composes every turn), the 2 x 68 = 136 consistency check no longer holds (68 + (68 - games) = 133), and the responder-side PASS is stated as NEEDING a real-key re-measurement rather than assumed to carry over. Measured: full suite 1293 passed / 96.35% coverage (05-06's own contribution is +11 tests; the total is JOINT with 05-05), ruff 0, line-limit exit 0, no-llm-in-strategy OK, GATE-6 re-run all three book Sec10.4 criteria PASS -- and its evidence JSON gained police_received and thief_received hint:5 where both previously read {commit:5, reveal:5}, which IS G3 at the integration level (restored with git checkout so the diff stays code-only, the 05-04/verify-work-6 convention). tests/integration/test_step0_and_audit.py verified UNMODIFIED by all three commits (verification item 6). THREE DEVIATIONS, all auto-fixed: two Rule-3 line-gate SPLITS (test_turn_hint_buffer.py hit 155/150 -> tests/unit/test_hint_freshness.py plus shared tests/unit/_hint_fixtures.py; test_turn_buffer.py hit 168/150 under the re-specification -> record_hint's four cases moved to test_hint_freshness.py, still calling the re-exported name) and one Rule-1 fix to a test THIS plan wrote (the inbound-record count was asserted exactly and failed 4 == 5; the last hint a side pushes can still be in flight when the RECEIVER's loop exits, so it is now a gap-free PREFIX at most one short, which is the channel being best-effort by design). THE COMMIT-ATTRIBUTION COLLISION: 05-05 executed in this SAME worktree on the SAME git index and its path-less git commit swallowed Task 2's six staged files into ead48df; content verified file-by-file, deliberately NOT rewritten (05-05 has already recorded that hash and a reset --soft would race a live executor), and Task 3 switched to git-commit---paths with no git add, which landed clean. Also hit twice: the pre-commit hook runs ruff over the WHOLE repo, so a parallel executor's half-written file blocks an unrelated commit -- waited both times, never --no-verify. ONE NEW DEFERRED ITEM (#4, logged not fixed): test_late_peer_teardown.py's non-vacuity test pins a 0.3 s real-socket race and flakes under load. Attribution MEASURED before blame -- pre-05-06 worktree 6/6 and 4/4 clean, HEAD 4/4 clean on the full integration suite, HEAD clean with AND without 05-06's outcome-is-None guard; both observed failures fell inside the window where a second pytest process was running. Knowledge graph refreshed (05-96: 7016 nodes / 12737 edges / 438 communities); graphify explain record_hint resolves to turn_hint_buffer.py L100 with both await_move and drain_trailing_hint still reaching it. test_gate4.py now 149/150 -- split before adding anything. NOTHING TICKED in ROADMAP.md (standing convention); docs/phases/phase-5/TODO.md row 05-06 marked in-progress with the three commit hashes. NEXT -- 05-07 (G5, keyless LLM made legible), then 05-08 (HUMAN remote round attempt 2, the only thing that can close GATE-5 criterion 2; both of its named blockers, 05-05 and 05-06, are now closed). PRIOR: PHASE 5 PLAN 05 EXECUTED (2026-08-14) -- G2 CLOSED ON THE CODE SIDE. The handshake-negotiated game id now governs the WHOLE match, not just a declaration filename, and D-60's committed state record finally has production readers. Proven on a real loopback game (dev_launch, exit 0): ONE stem f21a1071045f801c across BOTH sides' log + ledger + declaration (six artifacts, four of them the criterion-2 pair), both ledgers committing that same state.game_id, both sides ending audit_verdict matched=true, and every state.turn agreeing with its own ledger turn. Before this plan the thief's log/ledger kept its own process-local uid every single run -- the 2026-08-13 artifact (074fc2b16888899e vs d50ceb00be724b93) reproducible on loopback. Task 1 (ead48df): new src/pursuit/network/game_identity.py holding GameIdentity (a MUTABLE game_uid/log_path value object), negotiated_game_id (D-61's ONE policy definition) and the two NET-05/NET-07 JSONL sink closures RELOCATED out of agent_wiring.py (148/150, no room) and re-exported from it unchanged -- the same relocate-and-re-export move secret_wiring.py made in 05-02; agent_wiring.py came DOWN 148 -> 122. Both sinks take identity=None keyword-only: supplied they resolve the sink AT CALL TIME (they are bound at construction, design note 12, and cannot close over an AgentContext that does not exist yet -- which is exactly why the naive fix leaves them writing to the pre-negotiation path forever, silently); omitted they are byte-identical to before, so test_agent_lifecycle.py / test_agent_lifecycle_resilience.py / test_turn_resilience.py pass UNMODIFIED. AgentContext gains identity/negotiated_game_id/candidate_game_ids, all optional and defaulted. Task 2 (b06b4b3): adopt_negotiated_game_id called from run_agent immediately after "if not result.agreed: return None" and BEFORE write_declaration/run_turn_loop -- ledger_path derives from log_path.stem and commit_own_action first runs inside the turn loop, so the D-64 ledger needs no separate handling and there is nothing to rename; agent_audit_wiring._declared_game_id DELETED (grep "def _declared_game_id" src/ -> no hits) with write_declaration calling the one negotiated_game_id definition, which keeps a NON-adopting caller (the integration harnesses) on exactly the pre-05-05 filename. New tests/integration/test_game_id_negotiation.py hands the two sides DELIBERATELY DIFFERENT uids with log paths DERIVED from them and each side passing ITS OWN uid as local_game_id to both default_context and perform_handshake -- the existing harness takes ONE game_uid for both sides and hardcodes stems "a"/"b", so the property is literally unassertable there. Task 3 (01ff8ed): _audit_one now reads payload.state -- state.turn == entry.turn ALWAYS (free: commit_own_action passes ONE turn to both the record and the append), state.role != forbidden_role (a NEGATIVE check, never equality with our expected token: book Sec5.3.1 shares the RECIPE not the contents, and this repo alone carries two role vocabularies, so an honest peer writing "cop" must still match -- rules 16/22), and state.game_id IN this game's candidate ids (MEMBERSHIP, never equality: both equality shapes were tried and both false-accuse -- gating on the peer having PUBLISHED accuses a symmetric honest opponent on turn 1 of every game, and gating on THIS side having ADOPTED proves only that WE adopted and mutually technical-losses two honest implementations that merely swap conventions, while silently disabling the check in every game we play as police). Placement is EXACT: after the re-hash and BEFORE the trailing-commit early return, or a replayed record takes that exemption and skips the state check entirely. THE MEASURED CANDIDATE SET, both roles, with O = our minted uid and P = the peer's published id: police {O,P} TWO elements, thief {O,P} TWO elements AFTER adoption has rebound game_uid to P -- a set rebuilt from {game_uid, negotiated_game_id} after the rebind yields {P} on the thief and degenerates to equality, which probe B measured exactly (the artifact test and the POLICE half both still pass; only the thief half collapses). Peer published none -> None, check skipped; ids already equal -> {O}, legitimately ONE element, nothing asserts len==2 in production. ALL THREE limitations stated in source (audit_state.py), not implied: (a) a peer publishing NO game_id is not bound cross-game at all; (b) a peer publishing a PRIOR game's id defeats membership outright -- contained by _audit_one's LAST check (payload.move vs what was actually played, which obliges the attacker to have really played the old move), and explicitly NOT by verify_reveal, which re-hashes against whatever h_commit the peer chose to send this game so h_old + the matching old payload passes; (c) a peer deriving a THIRD id is accused under any candidate-set rule -- weighed and declined. THE 05-UAT.md G2 PROBE IS CLOSED: the forged {game_id: OTHER-GAME, turn: 99, role: police} record that reported "matched" now reports "committed state record names turn 99 (replay)" -- even with both optional arguments omitted, since the turn check needs no parameter. Revert probes all recorded verbatim in the SUMMARY (Task 2 probe A: no adoption -> two different stems and candidate set None; Task 2 probe B: the wrong-place set -> {P} on the thief only; Task 3: 7 substantive cases fail pre-fix while all 5 fairness controls plus the wiring assertion pass pre-fix). Measured: full suite 1293 passed / 96.35% coverage, ruff 0, line-limit exit 0, no-llm-in-strategy OK, GATE-6 re-run all three book Sec10.4 criteria PASS (evidence JSON restored, verdict fields byte-identical). THIS PLAN'S OWN CONTRIBUTION IS +20 TESTS, measured file by file -- the 1293 total is a JOINT number because ANOTHER AGENT WAS EXECUTING 05-06 CONCURRENTLY IN THIS SAME WORKTREE (see the SUMMARY's Issues section: no work was lost, but commit ead48df also swallowed their staged files via the shared git index, and Tasks 2/3 switched to "git commit -- <paths>" to stop it recurring). THREE DEVIATIONS, all auto-fixed and all documented: security/audit.py was SPLIT (new audit_state.py) at 168/150 rather than compressed, and test_audit_state_binding.py likewise at 153 (new test_audit_state_wiring.py); test_audit.py/test_audit_coverage.py's _genuine_records fixtures were made FAITHFUL to commit_own_action (a per-turn state record instead of one turn-1 record reused across turns) with no assertion weakened and no test deleted; and a malformed peer payload now becomes a named mismatch instead of a TypeError out of verify_reveal(**payload) that nothing upstream caught and that killed us before any verdict (rule 36, the class 06-06 fixed for ToolError). Knowledge graph refreshed (05-96: 6982 nodes / 12704 edges / 432 communities). agent_lifecycle.py now 148/150 -- logged as deferred item #2 with the suggested teardown split. NOTHING TICKED in ROADMAP.md (standing convention); docs/phases/phase-5/TODO.md row 05-05 marked in-progress with the three commit hashes. NEXT -- 05-07 (G5, keyless LLM made legible), then 05-08 (HUMAN remote round attempt 2, the only thing that can close GATE-5 criterion 2); 05-06 (G3+G4) was landing concurrently. PRIOR: PHASE 5 PLAN 04 EXECUTED (2026-08-14) -- G1 CLOSED ON THE CODE SIDE. The 2026-08-13 remote round's false accusation is now structurally impossible: run_final_audit takes the turn loop's board_outcome (wired at agent_entrypoint.py:101, FROM run_turn_loop at :91 -- the production wiring, not a dead branch), a failed OUTBOUND push with a board outcome standing writes one non-accusatory EventType.AUDIT_INCOMPLETE record naming OUR OWN send and FALLS THROUGH to the receive+audit steps instead of returning TECHNICAL_LOSS (rules 16/22 -- machine A's peer_audit carried machine B's five ledger hashes byte-for-byte at the exact moment B declared A unresponsive). All three genuine sanctions survive, each with a paired fairness control: no board outcome -> TECHNICAL_LOSS, a peer withholding its nonces (rule 36) -> TECHNICAL_LOSS, a genuine AUDIT_HASH_MISMATCH (D-67) -> TECHNICAL_LOSS, and the except ToolError branch stays accusatory on purpose (06-06). record_technical_loss now appends a CORRECTED game_over so the log's last outcome and main.py's exit code can never disagree; record_audit_verdict's mismatch tail DELEGATES to it rather than keeping a second copy. New sibling agent_teardown.linger_for_peer (64/150) is the bounded post-audit grace window -- total cap NetworkParams.response_timeout, quiet interval NetworkParams.backoff_seconds (Table 19 rows 6 and 3), ZERO numeric literals in the module and ZERO new keys in any config/*/network.json; watchdog_threshold deliberately not borrowed. shutdown_cleanly split into stop_watchdog/stop_runtime and KEPT as their composition; run_agent stops the watchdog BEFORE the linger (NET-07: touch() appears nowhere in the audit path -- PRE-EXISTING, FOUND here not introduced here -- and the freeze action is os._exit(1)) and the runtime AFTER it inside a try/finally (CancelledError is a BaseException; the three-bare-statement shape provably leaks the bound port). Capture mechanism: option (b), module-level helpers, so the unit order list and the integration harness name the SAME three steps. Task 3 shipped the first two-peer harness in this repo that binds REAL loopback sockets, SEQUENCES the two sides and actually tears down (tests/integration/late_peer_harness.py + test_late_peer_teardown.py) -- the in-memory Client(server) transport makes runtime.stop() a no-op, which is structurally why nothing caught this. Revert probes all recorded verbatim in the SUMMARY: Task 1 case 1 fails pre-fix on the substantive assertion (old code really returned TECHNICAL_LOSS for our own failed send) while controls 2 and 4 were already green; Task 2's cancellation test fails against three bare statements (order[-1] == 'linger_for_peer', stop_runtime never ran); Task 3's linger=False probe raises httpx.ConnectError out of B's run_final_audit so B ends with NO verdict at all -- the 2026-08-13 artifact reproduced on loopback -- and the reverse probe (linger=True) fails too, so the harness genuinely distinguishes the shapes. Measured: full suite 1262 passed / 96.30% coverage (baseline 1251 / 96.26%), ruff 0, line-limit exit 0, check_no_llm_in_strategy OK, GATE-6 re-run twice with all three book Sec10.4 criteria PASS (evidence JSON differs in exactly 3 timestamp lines, every verdict field byte-identical; restored so the diff stays code-only). Linger cost measured on a clean loopback game: dev_launch 17.44/17.64 s exit 0 with both sides recording audit_verdict matched=true, vs 14.4-14.7 s exit 1 without -- +2.8 to +3.0 s for the pair against a 5 s per-side quiet interval. ONE DEFERRED FINDING, logged not fixed (.planning/phases/05-cloud-exposure-and-tunneling/deferred-items.md #1): httpx.ConnectError escapes call_with_retry (RETRYABLE_TRANSPORT_ERRORS is exactly (McpError, DeadlineExpired), no catch-all by design) and KILLS the process on teardown, making US the side that published no nonces (rule 36). Pre-existing, measured 4/4 runs, closed on loopback by the linger but not in general; deadline.py is outside this plan's files and 06-06 affirmed its no-retry contract, so the fix needs its own plan -- route it to record_audit_incomplete, never to a technical win. Knowledge graph refreshed (05-96: 6827 nodes / 12372 edges / 429 communities). agent_lifecycle.py now 144/150 and tests/unit/test_agent_entrypoint.py 142/150 -- no headroom left in either. NOTHING TICKED in ROADMAP.md (standing convention); docs/phases/phase-5/TODO.md row 05-04 marked in-progress with the three commit hashes. NEXT -- plan/execute 05-05 (negotiated game_uid, G2), then 05-06 (G3+G4 hint flow + the 17.4 s responder stagger), 05-07 (G5), then 05-08 (human remote round attempt 2, the only thing that can close GATE-5 criterion 2). PRIOR: PHASE 6 CLOSED -- gate met, UAT 11/11, and ALL FOUR security findings from the two adversarial audits fixed: plan 06-05 (audit join key bound to local turn truth; caught mismatch made durable) and plan 06-06 (peer ToolError contained so it can no longer kill us before FINAL_REVEAL, rule 36; inbound envelope.sender validated, handshake exempt by design). Final measured state: full suite 1251 passed / 0 failed, 99.29% coverage, ruff 0, line-limit exit 0, GATE-6 re-run with all three Sec10.4 criteria PASS. deferred-items.md #1-#2 remain open by design (logging-granularity gap with equivalent evidence; correct rule-37 counter behaviour) -- neither a gate criterion. NEXT: plan Phase 7. PRIOR: (/gsd:verify-work 6, 2026-08-09) -- 9/11 UAT tests pass on evidence re-measured this session at HEAD=b3655348; tests 8 and 9 are ISSUES. A 5-lens adversarial audit found the mutual audit's join key is the peer's own declared envelope.turn, which nothing validates -- turn-skew converts every D-67 forgery into a benign "trailing commit" AND empties the rule-36 coverage intersection, reopening the empty-{"records": []} evasion; separately, a caught mismatch never reaches a durable outcome record (game_over is written before the audit and never corrected; main.py discards the overridden outcome). Both reproduced by hand against the shipped functions with paired controls. Detail in 06-UAT.md Gaps (measure_gate6.py re-run exit 0, all three Sec10.4 criteria PASS, evidence JSON differs from the committed one in exactly 3 timestamp lines with every verdict field byte-identical; full suite 1226 passed + the known load-sensitive timing flake re-confirmed passing in isolation at 0.18s; coverage 99.33%; ruff 0; line-limit exit 0; no-llm-in-strategy OK; security.json pair diff-confirmed byte-identical; 13 Phase-6 proof tests re-run green). ROADMAP.md Phase 6 + docs/phases/phase-6/TODO.md + root docs/TODO.md all ticked; graph refreshed at verify-work (6577 nodes / 11972 edges / 413 communities). UAT record: .planning/phases/06-security-and-cryptography/06-UAT.md. NEXT -- plan Phase 7 (reporting-and-visualization-shell). PRIOR: PHASE 6 PLAN 04 EXECUTED (216eec4, 2026-08-09) -- GATE-6 measured, all three Sec10.4 criteria PASS with real localhost evidence, zero env vars (scripts/measure_gate6.py + 6 gate6_*.py helpers), docs/PRD_commit_reveal.md written, GATE-6-MEASUREMENT.md written, knowledge graph refreshed (06-96). PHASE 6 IS NOW CODE+TEST+GATE COMPLETE (4 of 4 plans) and GOAL-VERIFIED (06-VERIFICATION.md status passed, 11/11 must_haves, gsd-verifier 2026-08-09) -- verify-work (triplet/ROADMAP ticking) still pending per house convention. Full suite 1226 passed / 1 pre-existing timing flake (confirmed passing in isolation) -- 1227 when the flake doesn't fire, matching the baseline exactly -- 96.27% coverage, ruff/line-limit/no-llm-in-strategy all clean. Two minor, non-blocking, out-of-scope findings logged (not fixed) in .planning/phases/06-security-and-cryptography/deferred-items.md (FINAL_REVEAL envelope-logging symmetry gap in agent_audit_exchange.py from 06-03; the measurement-vs-league games_played.json counter distinction). NEXT -- /gsd:verify-work 6 (tick ROADMAP.md Phase 6 + docs/phases/phase-6/TODO.md, confirm the phase triplet), then Phase 7 (reporting-and-visualization-shell) planning.
last_updated: "2026-08-14T17:55:00.000Z"
last_activity: 2026-08-14 -- Phase 05 plan 07 executed (05-07: G5 gap closure -- the keyless LLM made legible, wave 3, depends_on [05-05]). Two tasks, two atomic commits: 951d87d (feat, has_api_key + API_KEY_ENV_VAR promoted public + the one startup WARNING + compose's DEBUG fallback line + tests/unit/services/test_llm_client.py), 50ac2fe (feat, declared_llm_name + declare_step0 wired to it + the ten-key shape test + the first-person system template and STYLE_GUIDE bullet + docs/PRD_deception.md's verbatim quote + the PRD drift-guard test + the GATE-6 re-run evidence). The substantive judgement calls: WARNING not INFO (no logging.basicConfig exists, measured on a bare interpreter), DEBUG not WARNING for the per-turn fallback (it would bury the startup line), the resolved provider CLASS not the string "claude_api", and NO person validator -- a fuzzy false positive would silently divert a good hint to the bank, recreating the exact silent-fallback problem being closed. Full account, the four measured llm_name combinations, the three deviations and the gate6-evidence investigation in .planning/phases/05-cloud-exposure-and-tunneling/05-07-SUMMARY.md; self-check PASSED (15 files + 2 hashes verified, every number re-run at the final tree state rather than carried over). PRIOR -- Phase 05 plan 09 executed (05-09: deferred item #1 -- transport-failure containment, wave 3, depends_on [05-04]). Two tasks, two atomic commits: f31ece5 (fix, uv add httpx + RETRYABLE_TRANSPORT_ERRORS widened to httpx.TransportError + LocalProtocolError/UnsupportedProtocol joining the raise-first clause + the taxonomy split into deadline_errors.py + tests/unit/test_deadline_httpx.py), f602eb3 (fix, unwraps_to_retryable + error_evidence closing fastmcp's connect-path RuntimeError wrapper, the two-boundary containment proof, the wrapper-shape unit file, the REAL-SOCKET anchor, and deferred-items.md #1 closed / #5 accepted / #6 opened). The substantive finding: a widened tuple ALONE left the defect open, because the connect path wraps -- caught only by re-running the end-to-end harness and reading the peer's log, after five unit tests and a real-socket 403 anchor all passed against the wrong reading. Full account, all four revert probes verbatim, the thief's actual final records, and the measured test_late_peer_teardown.py runtimes in .planning/phases/05-cloud-exposure-and-tunneling/05-09-SUMMARY.md; self-check PASSED (10 files + 2 hashes verified, four claims RE-MEASURED at self-check time rather than carried over). PRIOR -- Phase 05 plan 05 executed (05-05: G2 gap closure -- one negotiated game id + a state record with readers, wave 2, depends_on [05-04]). Three tasks, three atomic commits: ead48df (feat, game_identity.py + GameIdentity + negotiated_game_id + the two relocated JSONL sinks + AgentContext's three optional fields + default_context's single shared identity), b06b4b3 (feat, adopt_negotiated_game_id wired into run_agent between result.agreed and write_declaration, _declared_game_id deleted, the new different-uids integration harness), 01ff8ed (feat, audit_state.state_binding_detail + the three checks placed after the re-hash and before the trailing-commit exemption + run_final_audit supplying both arguments per direction + the malformed-payload containment + the two fixture-fidelity corrections). Full account, every revert probe verbatim, the measured candidate set for both roles, all three stated limitations and the concurrency incident in .planning/phases/05-cloud-exposure-and-tunneling/05-05-SUMMARY.md; self-check PASSED (16 files + 3 hashes verified, verification items 6 and 7 re-run as greps at self-check time rather than carried over). PRIOR -- Phase 05 plan 04 executed (05-04: G1 gap closure -- verdict honesty + bounded teardown grace, wave 1, depends_on []). Task 1 (8f35721, fix): EventType.AUDIT_INCOMPLETE added additively exactly as AUDIT_VERDICT was (stable field set untouched); agent_audit_verdict.record_audit_incomplete reuses _verdict_record, the field assembly _technical_win_record now also calls -- one copy, two event types -- and carries the ladder's genuinely measured attempts/timeout/elapsed/last_error with a reason constant (_OWN_SEND_FAILED) deliberately NOT added to TechnicalWinReason, because the record is not a technical win and its subject is our own send; record_technical_loss appends the corrected game_over and record_audit_verdict's mismatch tail delegates to it; run_final_audit(ctx, *, board_outcome) with the four-way control flow; agent_entrypoint.py:101 passes board_outcome=outcome from run_turn_loop at :91 with an explicit comment on why the sibling except ToolError branch must STAY accusatory; test_step0_and_audit._run_audit_and_merge threads both outcomes so the tamper sibling and the four gate6_*.py scripts inherit the production shape unchanged; test_agent_audit_wiring.py's two cases re-specified (not loosened) as the board_outcome-is-None branch. Task 2 (6fd4fb9, feat): agent_teardown.py, linger_for_peer pulling through deadline.wait_for_opponent so deadline._bounded stays the ONE asyncio.wait_for site (QUAL-02), clock injected as a DI seam; agent_lifecycle gains stop_watchdog/stop_runtime with shutdown_cleanly kept as their COMPOSITION (its only remaining callers are three test files); run_agent's finally is stop_watchdog -> try linger finally stop_runtime. Task 3 (142c4b4, test): late_peer_harness.py + test_late_peer_teardown.py, real loopback sockets via test_secret_channel's own _free_port/_wait_until_accepting (imported, not re-copied), A audits then performs run_agent's three steps while B arrives late; the non-vacuity probe PINNED as a permanent second test asserting the durable property ("B's own push did not land") rather than today's exception type, so it survives the deferred ConnectError fix. Three Rule-2 test-strength additions beyond the plan's literal text, all documented. 05-04-SUMMARY.md written; self-check PASSED (17 files + 3 hashes verified, one line-number citation corrected by re-measurement rather than carried over). PRIOR (2026-08-09) -- Phase 06 plan 04 executed (06-04: GATE-6 measurement + docs, wave 4 of 4, depends on 06-03 -- the LAST plan in Phase 6). Task 1 (77eafff): scripts/measure_gate6.py + 6 gate6_*.py helper modules (gate6_common/gate6_clean_game/gate6_declarations/gate6_tamper/gate6_step0/gate6_report, each under the 150-code-line house limit, mirroring the gate4_*/gate5_* precedent -- scripts/ is outside the coverage gate but the limit is still honored by hand). Reuses the SAME real two-peer harness 06-03's own tests already use (tests/integration/test_step0_and_audit.py's _play_to_turn_loop_end/_run_audit_and_merge, imported directly -- the ALREADY-established sibling-test-import precedent test_step0_and_audit_tamper.py itself uses) rather than the plan's own stale key_link (two_peer_game.py's play_two_peer_game cannot carry the Step-0 params 06-03 added). Criterion 1 (clean game): commit/ack/reveal counted 5/5/5 both sides, zero nonce occurrences in either wire-mirroring JSONL against 5 nonce-bearing ledger records per side, the D-58 both-locked-gate ordering holding with zero violations, both declaration files present and predating first move content, an honest barrier count (1 this run, not forced). Criterion 2 (tamper): both D-67 tamper classes re-run live -- (a) corrupted ledger payload fails the re-hash check, caught on both sides' own self-audit too; (b) THE D-67 case, hash/payload untouched (independently re-verified) but the observed action differs, caught by check 3 alone. Criterion 3 (Step-0): a live digest forgery, evaluated through the real respond_to_handshake against a real default_context-built AgentContext (not a full two-server round trip, since a forged digest is only detectable from the RECEIVING side's own evaluation -- documented in gate6_step0.py's own docstring) -- HandshakeOutcome.STEP0_MISMATCH fires, State.ERROR, move 1 confirmed unreachable via an explicit post-abort machine.attempt(State.MY_TURN) call. All three criteria PASS, evidence in docs/phases/phase-6/gate6_measurement_evidence.json; script confirmed idempotent across two consecutive runs, zero env vars set (ANTHROPIC_API_KEY explicitly cleared, mirroring the test suite's own monkeypatch.delenv). Task 2 (1de0dcf): docs/PRD_commit_reveal.md, mirroring docs/PRD_mcp_transport.md's house structure exactly -- mechanism/scope with the SEC-01..08 requirements table (grep-confirmed all eight IDs present), topology/design (D-58 four phases + initiator/responder asymmetry, D-59/D-66 composite action dict + hash recipe, D-64 nonce ledger, D-62/D-63 Step-0 two-layer verification, D-67 audit design), interfaces copied verbatim from the 06-01/02/03 SUMMARY files, out-of-scope future-phase extensions, parameters traced to 06-PLAN-OUTLINE.md Sec2. Task 3 (216eec4): docs/phases/phase-6/GATE-6-MEASUREMENT.md (the three Sec10.4 criteria quoted verbatim, one section per criterion with method/run-command/what-a-PASS-looks-like/measured evidence, every claim pointing at a named evidence-JSON field; two honest findings documented rather than smoothed over -- FINAL_REVEAL is not itself logged as a message_sent/message_received envelope record, audit_verdict is the correct evidence instead; measurement games advance the real games_played.json counter, same as 06-03's own pytest runs) + knowledge graph refresh (06-96: graphify update ., 6510 nodes/11909 edges/408 communities, built at commit 1de0dcf7, graph.html skipped over the 5000-node viz limit matching the 04-12/05-03 precedent -- gitignored regardless, only the committed GRAPH_REPORT.md moved, 566 lines changed, git diff --stat confirms a real change). Two minor, non-blocking findings logged (not fixed, out of this plan's scope per the SCOPE BOUNDARY rule) in .planning/phases/06-security-and-cryptography/deferred-items.md. All 3 tasks committed atomically, pre-commit hook passing every time, never bypassed. Full suite re-measured: 1226 passed / 1 pre-existing timing flake (test_belief_policy.py's own per-turn-budget test, confirmed passing in isolation, 0.22s, unrelated to this plan) -- 96.27% coverage, ruff/line-limit/no-llm-in-strategy all clean. 06-04-SUMMARY.md written. Self-check PASSED (all 11 files + 3 commit hashes verified present). NOTHING TICKED anywhere -- ROADMAP.md Phase 6 checkboxes and docs/phases/phase-6/TODO.md left unchecked, per this project's own standing convention (05-03/06-01/06-02/06-03 all did the same) -- that is /gsd:verify-work 6's job. PHASE 6 IS NOW FULLY CODE+TEST+GATE COMPLETE. PRIOR (2026-08-09) -- Phase 06 plan 03 executed (06-03: Step-0 + Final-Reveal mutual audit, wave 3 of 4, depends on 06-02). Task 1 (54048e3): uv add psutil; step0_collect.py's collect_declaration gathers the full book Sec5.5 field set (OS/CPU/RAM via platform+psutil, GPU best-effort via nvidia-smi subprocess with an honest not-detected on any failure, LLM name/code version/team code/games-played-so-far, exact git commit hash via subprocess, raising loudly on failure) plus read_games_played/record_game_played over durable_write's crash-safe pair (rule 37/38); step0_sign.py's digest_declaration/sign_declaration/verify_declaration (D-62 -- digest always, HMAC only when a shared secret exists, explicit signed:false never silently verified). Task 2 (7bb130b): HandshakeKey.STEP0_DIGEST/GAME_ID extend build_offer's payload the same omit-dont-null way SCENT_DIGEST does; HandshakeOutcome.STEP0_MISMATCH aborts before move 1 via the SAME State.ERROR seam (state_machine.py untouched); HandshakeResult.peer_game_id read unconditionally (D-61). A REAL DESIGN CORRECTION (Rule 1) made here: the plan's own literal text specified comparing local vs remote step0 digest for EQUALITY via compare_named_digest, mirroring SCENT_DIGEST -- but a Step-0 declaration is inherently per-agent (digests role among other fields), so two different roles' declarations can never hash equal; implementing this literally would abort EVERY real two-role game the instant both sides opt in. Fixed: _step0_present() checks PRESENCE only, matching rule 24's actual failure mode (Step-0 never ran on the peer's side). test_handshake_step0.py's own load-bearing D-61 test uses deliberately different local_game_id values on both sides, proving negotiation genuinely resolves to the initiator's value. Task 3 (ed48ee4): audit.py's audit_peer_records/AuditRecord/all_matched -- three ordered checks per turn (observed commit present, re-hash matches H_commit, revealed action equals what THIS side actually saw played in-game); TechnicalWinReason.AUDIT_HASH_MISMATCH and EventType.AUDIT_VERDICT added (both additive). Unit tests prove BOTH D-67 tamper classes distinctly: a flipped payload field fails the hash check, and -- the D-67 case itself -- hash/payload left completely untouched (independently confirmed still verifies) but the claimed action differs from what was actually played, caught by check 3 alone. Task 4 (be75519): agent_audit_wiring.py (declare_step0/write_declaration/run_final_audit) + agent_audit_exchange.py (a THIRD sibling, pre-authorized: FINAL_REVEAL push/receive via the existing call_with_retry ladder, observed-history extraction from the wire log, verdict recording reusing the EXISTING TechnicalWin dataclass, never a parallel verdict type) wired into agent_entrypoint.run_agent's three new call sites -- stays a thin caller. agent_lifecycle.py ALSO needed editing (Rule 3, not pre-authorized in this plan's files list): default_context had to thread local_step0_digest/local_game_id into make_handshake_responder, the SAME seam local_scent_digest already uses, or Step-0 would never reach the real inbound responder path. tests/unit/test_agent_entrypoint.py fixed for the new call-site fallout (Rule 3). games_played.json gitignored (mutable per-role runtime counter, not fixed config). Full real two-peer integration proof: declaration files land on both sides sharing the game_id BEFORE any move content is logged, a clean game's audit_verdict record shows matched:true on both sides, and BOTH D-67 tamper classes are caught live -- corrupting police's own ledger payload makes thief's audit fail (AND police's own self-audit also fails, symmetric honesty proven, not just asserted) while corrupting only what thief itself observed played in-game (ledger/hash left genuinely untouched, independently confirmed) fails via check 3 specifically, proving the hash-only bypass is genuinely closed. Rule-2 coverage-closing tests added for every new module's untested branch (agent_audit_exchange.py/agent_audit_wiring.py/step0_collect.py all reach 100%). All 4 tasks committed atomically, pre-commit hook passing every time, never bypassed. 06-03-SUMMARY.md written carrying the declaration filename convention, HandshakeResult.peer_game_id contract, and the AuditRecord shape verbatim for 06-04. Self-check PASSED (all 13 files + 4 commit hashes verified present). NOTHING TICKED anywhere -- ROADMAP.md Phase 6 checkboxes left unchecked, docs/phases/phase-6/TODO.md not yet created. Knowledge graph refresh (06-96) still pending -- not run this plan. handshake_evaluate.py (149/150) and agent_wiring.py (146/150) now have essentially no line-count margin left, flagged for 06-04. FOLLOW-UP (10f3a26, coordinator-directed post-review): the presence-only digest fix left step0_sign.verify_declaration with ZERO production callers -- only the digest crossed the wire, so a peer sending any 64-char string passed Step-0. Closed: HandshakeKey.STEP0_DECLARATION (a fifth, opt-in payload key) now carries the FULL published declaration (book Sec5.5 -- there is no secrecy reason to withhold OS/CPU/RAM/GPU/model/code-version/commit-hash); handshake_step0.py (new sibling -- handshake_evaluate.py had zero line-count room left, now sits EXACTLY at 150/150) verifies the peer's content against its own claimed digest (+ HMAC when both sides hold the shared secret) via step0_sign.verify_declaration whenever the peer opts in to send it. A digest-only peer (an opponent's own implementation we cannot force to publish content) still agrees; a declaration mutated AFTER its digest was computed aborts before move 1 with a non-accusing report. HandshakeResult gains peer_step0_declaration (read unconditionally); write_declaration also persists the peer's own declaration (declaration_<game_id>_peer.json) for Phase-7 auditability. agent_lifecycle.py's default_context now resolves the shared secret ONCE and reuses the same tuple for both PeerRuntime's middleware and the responder's Step-0 verification. 4 new tests (content matches digest agrees; content tampered after its digest was computed aborts before move 1, house style checked against accusing language; digest-only peer still agrees; HMAC mismatch on a wrong local secret also aborts) plus the integration harness now threads real declaration content end to end and asserts the peer file is persisted. grep confirms verify_declaration now has exactly one production call site. Full suite re-measured: 1222 passed, 96.26% coverage. 06-03-SUMMARY.md updated to describe the exchange-and-verify design, superseding its presence-only-only description. FOLLOW-UP 2 (4ac475a, coordinator-directed post-review): all_matched([]) is vacuously True and audit_peer_records only ever audited entries the PEER chose to include -- nothing checked that peer_records COVERED the turns actually watched played, so an opponent sending FINAL_REVEAL {"records": []} (the cheapest possible rule-36 evasion) passed the mutual audit and kept its board outcome; run_final_audit had no coverage check anywhere. Closed: audit_peer_records now requires every turn present in BOTH observed_commits AND observed_reveals (fully exchanged -- committed AND revealed in-game) to appear in peer_records; a missing turn is a named mismatch. The SAME change fixed a real false-accusation bug in the opposite direction: a turn with an observed commit but no observed reveal is a legitimately TRAILING turn (CommitLedger.append runs BEFORE the REVEAL send, so an honest peer's own final reveal can contain a committed-never-revealed entry from an abnormal ending) -- now matched=True once commit+hash check out, never misbranded a forger. A genuinely turn-less game stays vacuously matched. No caller-side change needed; the self-audit direction gets identical semantics automatically. handshake_evaluate.py confirmed untouched (git diff empty), per the coordinator's own constraint. Tests: omitting one fully-exchanged turn mismatches exactly that turn; sending empty records while N turns were observed produces N mismatches (the evasion closed); an honest trailing commit-without-reveal is now matched=True (the false-accusation case fixed); a real two-peer integration case truncates one turn out of one side's ledger before the Final-Reveal exchange and confirms the other side's audit reports AUDIT_HASH_MISMATCH and Outcome.TECHNICAL_LOSS. Full suite re-measured: 1227 passed, 96.27% coverage. 06-03-SUMMARY.md updated again to describe the coverage check and the trailing-commit fairness fix.
progress:
  total_phases: 8
  completed_phases: 4
  total_plans: 62
...
Phase: 05 (cloud-exposure-and-tunneling) — **IN PROGRESS, gap closure.**
  8 of 9 plans executed (05-01..05-07 + 05-09). **05-07 (2026-08-14) closed G5**,
