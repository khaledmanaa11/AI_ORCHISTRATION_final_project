...
**Current focus:** Phase 06 CLOSED (gate met, both verify-work gaps fixed by 06-05) → next up Phase 07 — reporting-and-visualization-shell
...
Phase: 06 (security-and-cryptography) — **CLOSED.** 5 of 5 plans executed
  ...
  STILL OPEN, logged not fixed (deferred-items.md #3-#4, both verified by
  hand): an uncaught ToolError escapes call_with_retry and can kill the
  agent mid-game AFTER its ledger append but BEFORE any FINAL_REVEAL --
  making US the side that published no nonces (rule 36); and
  tools.py::_accept never checks an inbound envelope's `sender` against
  the opponent's role. Neither is a Sec10.4 criterion; both want a
  decision about which peer faults become a technical loss before league
  play.
