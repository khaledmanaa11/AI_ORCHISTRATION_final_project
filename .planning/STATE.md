...
status: executing
stopped_at: PHASE 5 PLAN 16 EXECUTED (2026-08-16) -- DEFERRED ITEM #10 CLOSED, AND THE ITEM UNDER-COUNTED ITS OWN EXPOSURE BY TWO LADDERS. Task 1 (a010a55, test, committed RED): three ladder cases run against unmodified source measure the hole in this repo's own numbers, on an injected clock in 0.000 s of real time -- `config: response_timeout=30 retry_count=3 backoff=5 watchdog_threshold=60`, `attempt cost = 35`, and `wait/push/move attempts=2 injected_elapsed=70.0s touches=0 checks=[False, True] fired=['freeze','exit'] killed=True verdict=None` on every leg. `killed=True` is not a flag the test sets: `LethalWatchdog` RAISES where production calls `os._exit(1)`, so `verdict=None` is a MEASURED fact rather than an inference from a boolean -- the D-13 ladder that would have answered at t=140 s never got to speak, and in production that second poll ends the process MID-GAME with our nonces ledgered and no FINAL_REVEAL sent (rule 36 against US) while the peer records `opponent_unresponsive`. Task 2 (4e3d42e, fix): the four `wait_for_*` legs pass `on_attempt=ctx.watchdog.touch` -- 05-13's own hook, used by the callers it deliberately left at the default -- and the same per-attempt touch opens the closure of `turn_buffer.await_move`, `turn_buffer.send_hint`, `turn_commit_send.push`, `turn_commit_send.send_move_only` and `capture_declaration`. AFTER: `attempts=4 injected_elapsed=140.0s touches=5 checks=[F,F,F,F] fired=[] verdict=TechnicalWin` on every leg -- 140 s of ladder against a 60 s threshold, zero freeze polls firing, an honest verdict recorded, widest touch gap `response_timeout + backoff_seconds` = 35 s < 60 s. THE FINDING: deferred #10 names the four `wait_for_*` legs and there were SIX unmarked ladders. The two it missed are the ones that matter -- a stalled peer stalls our PUSH FIRST, because `turn_commit_send.push` is what sends the COMMIT the wait leg is waiting for a reply to, and 05-15's `capture_declaration` runs INSIDE `run_turn_loop` where the watchdog stays armed to `agent_entrypoint:134`, so an unmarked ladder there killed us BEFORE `run_final_audit` could publish our nonces. Fixing only the named legs would have left the process dying at the identical t=60 s one door earlier, with the plan's own objective false and every test green. `grep -rn watchdog.touch src/` now returns 18 = 12 calls + 5 `on_attempt` hook passes + 1 comment; the call-form grep alone returns 12 and undercounts the hooked legs by five, so BOTH numbers are stated (05-13's deviation-6 lesson). NO NUMERIC VALUE MOVED ANYWHERE and `turn_commit_wait.py` is unchanged at 145 code lines -- the four edits are inline keyword arguments; the stale `:68-77` comment block, which stated as FACT that only `receive_final_reveal` passes a hook, is corrected in place and now carries the measurement. Task 3 (0993b05, test): eight cases over two files plus the harness. TEN PROBES, every one with a real count -- P1..P6 each revert ONE leg and fail EXACTLY one case (1 failed / 7 passed, six times: the pinning is surgical, not diffuse); W1 THE WRONG FIX of widening `watchdog_threshold` 60 -> 150 fails 6 OF 8 on `assert 140.0 > 150` / 'a Table-19 NUMBER was moved', because every ladder case reads the bound from config and asserts the ladder OUTLIVES it, which is precisely what tells 'you marked the attempts' apart from 'you moved the number'; W1b the same wrong fix ALONE (every touch reverted too) fails the same 6; W2 THE WRONG FIX of a blanket `ctx.watchdog.stop()` across the turn loop fails 8 OF 8, including `test_a_genuinely_frozen_turn_loop_is_still_killed` ('DID NOT RAISE ProcessKilledError') and `test_the_turn_loop_never_disarms_the_watchdog` ('the turn loop left the watchdog disarmed') -- that is the discrimination this plan owed, NET-07 preserved and PROVEN preserved rather than asserted in prose; and P7 PROBES MY OWN HARNESS, the check 05-13/05-14/05-15 each caught themselves needing -- strip `self._armed.check()` out of both stalled peers and 7 of 8 cases fail on 'NET-07 killed the ladder mid-flight', so the leading `armed.checks` conjunct is a falsifiable anti-vacuity guard and the suite is not certifying its own silence. `LethalWatchdog` calls `super().check()` FIRST and unconditionally, so 05-13's stop-flag gate (`da58bc2`) still runs and the hole that plan found is NOT reintroduced. ONE ASSERTION'S HONEST LIMIT RECORDED RATHER THAN LEFT FOR A REVIEWER: under `LethalWatchdog`, `armed.fired == []` and the `not any(armed.checks)` half cannot fail in these cases because a fired freeze raises first -- redundant, not vacuous; the load-bearing facts are that the call RETURNED at all, that `touches >= attempts` (what W2 fails on) and that the ladder outlived the threshold (what W1 fails on). Zero `parametrize` in all three new files, so pytest's empty-set-is-a-SKIP trap cannot apply; `assert (opponent, verdict) == ('h', None)` is ONE TUPLE deliberately, so neither half can shadow the other (05-15's finding applied pre-emptively). Production callers grepped for every changed name: all four `wait_for_*` legs are called from `turn_commit.py` (:79, :111, :151, :164), `on_attempt=` has 5 production call sites and NO caller is left at the default. THREE HELPERS EXTRACTED AT THEIR SECOND COPY (`armed_from`/`attempt_cost`/`table19_overrides` into `_fakes_watchdog.py`) with `test_audit_watchdog.py`'s four cases BYTE-UNEDITED and that file 146 -> 142, off deferred #11's list. TWO SPLITS, never compressions (`_turn_loop_fixtures.py` 74 and `test_turn_push_watchdog.py` 85, leaving `test_turn_loop_watchdog.py` at 110) -- the split ADDED assertions rather than removing any. A FOURTH DEVIATION WORTH THE EMBARRASSMENT: `test_late_peer_teardown` failed 4/4, far worse than its documented flake, and the cause was MINE -- an earlier scratch probe searched upward for `pyproject.toml` from the scratchpad, never terminated at the filesystem root, survived a `kill %job`, and had burned 680 s of CPU pegging a core, which is exactly the load deferred #4 is documented as sensitive to. Killed, then re-attributed by PAIRED measurement: 1 failed / 2 passed over three runs WITHOUT this plan's source changes and 1 failed / 2 passed WITH them, identical, on the item's own verbatim message -- and the failing run is reliably the FIRST run after a source file changes. Recorded in the item with the table; NOT fixed (05-04's file, and the only quick repair widens a timing constant). GATES: ruff 0, line-limit exit 0, no-llm OK, suite 1523 passed / 96.62% against the 1516 / 96.62% baseline (+8, exactly the eight added, coverage unchanged), all four changed production modules at 100% (turn_commit_wait 67/67, turn_buffer 57/57, turn_commit_send 47/47, capture_declaration 38/38), GATE-6 all three PASS with the evidence RESTORED not committed (3 timestamp/mtime lines plus 05-15's two `game_over` counters, every verdict field byte-identical), `dev_launch.py` exit 0 with ONE shared uid `0f803ac4058d208a` across both logs AND both filenames, both sides `audit_verdict matched=true`, ZERO `technical_win` and ZERO `watchdog_incident`. Knowledge graph refreshed: 7832 nodes / 14141 edges / 479 communities (was 7719 / 13861 / 482). Deferred #13 explicitly NOT touched, per this plan's own non_goals, WITH 05-14's measurement left standing. Nothing weakened, nothing skipped, no --no-verify.
prior_stopped_at: PHASE 5 PLAN 15 EXECUTED (2026-08-16) -- G10 CLOSED, and building the thing the plan called a "de-risk" is what exposed a PRE-EXISTING false-accusation path in `receive_final_reveal`. The full task-by-task account, all eleven probes and the shadowed-assertion self-audit finding are in .planning/phases/05-cloud-exposure-and-tunneling/05-15-SUMMARY.md; the previous verbatim narrative was rotated out of this field when 05-16 landed.
last_updated: "2026-08-16T22:45:00.000Z"
last_activity: 2026-08-16 -- Phase 05 plan 16 executed (05-16: deferred item #10, the turn loop's own 135 s-vs-60 s watchdog exposure; wave 8, depends_on [05-13]) -- ORCHESTRATOR-WRITTEN GAP-CLOSURE PLAN, not from /gsd:plan-phase. Three tasks, three atomic commits: a010a55 (test, the RED measurement), 4e3d42e (fix, every bounded attempt marked), 0993b05 (test, eight cases and ten probes). Three files created, seven modified. FOUR deviations auto-fixed (2 measurement-forced scope corrections, 1 blocking line gate, 1 self-inflicted tooling bug). HEADLINE: the deferred item under-counted its own exposure by two ladders -- a stalled peer stalls our PUSH first, so marking only the four named `wait_for_*` legs would have left the process dying at the identical t=60 s one door earlier with every test green. Six ladders marked. BEFORE: `attempts=2 elapsed=70.0s touches=0 checks=[False, True] fired=['freeze','exit'] verdict=None`; AFTER: `attempts=4 elapsed=140.0s touches=5 checks=[F,F,F,F] fired=[] verdict=TechnicalWin`. BOTH tempting wrong fixes refuted by named probe with real counts: widening `watchdog_threshold` 60->150 fails 6 of 8 ('a Table-19 NUMBER was moved'), a blanket `ctx.watchdog.stop()` fails 8 of 8 including the NET-07 frozen control. P7 probes my own harness (7 of 8 fail when the freeze poll is stripped out), which is the check the three prior plans each caught themselves needing. Suite 1523 passed / 96.62% (baseline 1516 / 96.62%), all four changed modules at 100%; GATE-6 all three PASS; dev_launch exit 0, one shared uid `0f803ac4058d208a`, both sides matched, zero technical_win and zero watchdog_incident. Knowledge graph refreshed: 7832 nodes / 14141 edges / 479 communities. The one suite failure is deferred #4's late-peer flake, re-attributed by paired measurement and NOT fixed.
prior_last_activity: 2026-08-16 -- Phase 05 plan 15 executed (05-15: the declaration story, settled and written down; wave 7) -- THE LAST PLAN OF THE ORIGINAL PHASE-5 SET. Five commits; building the plan's own "de-risk" exposed a pre-existing false-accusation path (`receive_final_reveal` read `records` off whatever arrived first). Eleven probes. Suite 1507 passed / 96.62%. See 05-15-SUMMARY.md.
progress:
  total_phases: 8
  completed_phases: 4
  total_plans: 63
