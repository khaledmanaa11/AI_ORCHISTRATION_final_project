...
Phase: 05 (cloud-exposure-and-tunneling) — **GATE-5 MET; follow-up gaps open.**
  ...
  **SELF-AUDIT caught a justification of MY OWN that was false** (`take_final_reveal`
  ...
  NEXT: `/gsd:verify-work 5` — **all 17 plans are executed** (05-01..05-17).
  **THIRTEEN DEFERRED ITEMS OPEN** — **#4 and #10 are now CLOSED** (the late-peer
  control got a deterministic happens-before gate on 2026-08-17; the turn loop's
  watchdog exposure closed by 05-16) and **#16, #17, #18 are NEW from 05-17**:
  **#16** the linger's quiet interval is derived from the wrong clock (5 s quiet,
  30 s cap, 35 s worst-case arrival→retry — the retry lands after the linger has
  returned); **#17** the linger drains through a plain `queue.get()`, so 05-17's
  buffer does NOT cover it (`queue 1 → 0`, `buffered None`, nothing logged);
  **#18** the police branch's bare pull turns an early FINAL_REVEAL into a
  technical loss. Also open: (**#14** (05-15) each dropped envelope costs the
