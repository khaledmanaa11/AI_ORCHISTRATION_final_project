...
- [x] `test_peer_roundtrip.py` passes — geometric message decoded correctly (GATE-1)
- [x] `test_turn_isolation.py` passes — two config roots, no shared runtime state (GATE-2)
- [x] `test_turn_lifecycle.py` + `test_turn_resilience.py` pass — state machine, illegal-transition reporting, watchdog + deadline (GATE-3)
- [x] Real two-terminal standalone launch: both peers start, handshake, exchange a move (ran to a full SURVIVAL outcome)
- [x] `uv run pytest --cov=pursuit` ≥ 85% (QUAL-10) — 96.87%
